...
Strong induction is a form of mathematical induction where instead of assuming \(A(n)\) is true in our 
...
\textbf{Inductive hypothesis:} \(A(j)\) for \(2 \le j \le n\) is true.

Consider the cases \(k + 1\) is prime or composite. 

In the second case then \(A(k + 1)\) is true.

For the first case, by definition is \(k + 1\) is composite it can be written as a product of factor \(a,b,c,dots\) where  
\(2 \le a \le b \le c \le \dots < k + 1\). By induction these number can also be written as the product of primes, and so on until we arrive at our 
base case. 
