...
\begin{itemize}
	...
	\item \(f\) is injective if there exists a function \(g: N \to M\) with the property \(g \circ f = \text{Id}_M\). Each 
		\(g\) is surjective and called the \emph{left inverse} of \(f\).

	\item \(f\) is surjective if there exists a function \(\gamma: N \to M\) with the property \(f \circ \gamma = \text{Id}_N\). Each 
...
Therefore such a \(n\) can not exists because of our conditions.
...
\begin{itemize}
	...
	\item \emph{Total relations:} There are \(2^{n \times m}\) total relations from \(A\) to \(B\). Each of the 
		  \(n \times m\) possible ordered pairs \((a, b)\) can either be included in or excluded from the relation.
