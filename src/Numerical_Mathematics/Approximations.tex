...
Hence, we want to minimize the expression 
...
where \(A^T A \in \R^{n \times n}\) is regular.

\textbf{Proof:}

We first want to show that \(A^T A\) is regular; hence, we do the following: 
...
If we assume there exists another solution \(\tilde{y} =  \tilde{x} + v, \ v \in \R^n, \ v\ne 0\), then 
...
Given \(A \in \R^{m \times n}\), then there exist matrices \(U \in \R^{m \times m}, \ V \in \R^{n \times n}\) such that 
...
For \(\alpha > 0\), \(A_{\alpha} = U \Sigma_{\alpha} V^T\) with
...
From this definition, we also get the fact that \(\kappa_2 (A_{\alpha}) \le \frac{1}{\sqrt{\alpha}}\).
...
By combining them in an expression, we get 
...
Formally, for \(A \in \R^{m \times n},\ \alpha > 0\), 

\[
    x_{\alpha} = (A^T A + \alpha I)^{-1} A^T b
\]

is the \emph{Tikhonov regularization} of \(x = A^+ b\) with parameter \(\alpha\).