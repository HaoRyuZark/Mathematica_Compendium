...
roots of a non-linear polynomial. For this, there are plenty of theories and methods.
...
that is \(f(a_0) f(b_0) < 0\), then there must be a root in between (Intermediate Value Theorem).
...
then we repeat the process. The termination criterion is the number of iterations set for the computation.

\subsection{The Regula-Falsi Algorithm}

This is a variation of the \emph{bisection algorithm} which improves the convergence velocity of the 
algorithm. 
...
that is, \(f(a_0) f(b_0) < 0\) implies there must be a zero in between.
...
We use the point at which our secant crosses the \(x\)-axis as our new estimation, giving us the new iteration rules:
...
This is again a slight modification of the previous algorithm given \(x_{ - 1}, x_{0}\). 
...
Taking the Taylor series of our function \(f\) and only considering the first linear term, we get 
...
If we were to stop at the quadratic term, we would get:
...
This algorithm comes from the idea that when taking a derivative if its slope is negative, then we need to move in the 
positive \(x\) direction: \(x_{k + 1} = x_k + \alpha f'(x_k)\) and if the slope is positive, then we move in the negative direction 
...
\(f'(x)\) is monotonically increasing, therefore
...
If we have the case that \(x\) is a zero root of \(f(x), f'(x), \dots\) or even leads to undefined expressions, then we can write \(f\) as 
a Taylor polynomial 
...
where \(q - 1\) is the degree of the last derivative such that \(f'(x) = 0, f''(x) = 0, \dots, f^{(q - 1)} = 0, f^{(q)}\ne 0\).
