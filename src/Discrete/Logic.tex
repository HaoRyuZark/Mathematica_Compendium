...
\emph{Propositional logic}, also called Boolean logic is one the foundational branches of mathematics, and thus, the 
first chapter of this book. In propositional logic, we use symbolic variables to represent the logic, and we can use any symbol for a representing a proposition, such 
\(p,q,r,s,t,\) etc. Propositions can be either \emph{true} (\(1\), T) or \emph{false} (\(0\), F) but it cannot be both.

\textbf{Remark}: In programming languages like C, any other number greater than 0 is treaated as T, w

\section{Predicates}

A \emph{predicate} is statement which contains variables linked by some property stated in the language of formal logic whose value can be true or false.
...
As a side not for the \emph{implication} we can think about it as \emph{if, then}. For the cases 
false, true; and false false; we call them \emph{vacously} true. From a false premise can come everything out.
