...
\begin{itemize}
	...
	\item Full replacement: \([y/x]\forall z((z = x) \land (y \in x))\), converted to \(\forall z((z = y) \land (y \in y))\),

	\item Partial replacement: \([y/ x]x = y \land \forall x (x \in y)\), the \(x\) of \(\forall x(x \in y)\) is not a free variable, hence it can not be
...
\subsubsection*{Universal Bound Laws}
...
\subsubsection*{Complement Law}

\[
	p \lor \neg p \iff \text{T} \qquad p \land \neg p \iff \text{F}
\]

\subsubsection*{Biconditional Law}
...
\subsubsection*{Indirect Proof (Contrapositive)}

\[
	(p \implies q) \iff (\neg q \implies \neg p)
\]

\subsubsection*{Disjunctive Syllogism (Disjunctive Exclusion)}
