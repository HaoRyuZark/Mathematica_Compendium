\newpage
\chapter{Relations, Maps and Functions}

\subsection{Relations}

Given two classes \(A\) and \(B\) a \emph{relation} \(R\) is subset of its Cartesian product \(R \subseteq A \times B\). 
...
\subsection{Diagonal Graph}

Given a relation, the graph \(I_A\) is defined as: \(\{(x, x) \mid x \in A\}\) and 
denoted as the \emph{diagonal graph} of \(A\).

\subsubsection*{Lemmi for Proofs}
...
A relation is called an \emph{order relation} \(R \subseteq A \times A\) if it is reflexive, antisymmetric and transitive.

\subsubsection*{Inverse Order Relation}

Given an order relation \(G\) on \(A\), then \(G^{-1}\) is also an order relation on \(A\).

\subsection{Strict Order Relation}

A relation is called a \emph{strict order relation} \(R \subseteq A \times A\) if it is irreflexive, antisymmetric and transitive.

\subsubsection*{Inverse Strict Order Relation}

Given a strict order relation \(G\) on \(A\), then \(G^{-1}\) is also a strict order relation on \(A\).

\subsection{Equivalence relation}

An \emph{equivalence relation} is a relation \(R \subseteq A \times A\) that is symmetric, transitive and reflexive.
...
\subsubsection*{Inverse Equivalence Relation}
...
Let \(\sim\) be an equivalence relation on a set \(A\) and \(a \in A\), the 
\emph{equivalence class} of \(a\), denoted by 
...
\subsection{Lemmi about Equivalence Classes}

For \(a,b \in A\) then 

\begin{itemize}

	\item \(aEb \iff [a]_E = [b]_E\)
...
\(aEb, x \in [a]_E \text{ tells us that also } x E b \implies x \in [b]_E \). For the other direction 
using the reflexive property we get \([a]_E = [b]_E \implies a E a \implies a \in [a]_E = [b]_E\)

\QED

\textbf{Proof II:}

\(x \in [a]_E \cap [b]_E \implies x E a, x E b, a E x \implies a E b\) a contradiction. 
Now, for \( [a]_E \cap [b]_E = \emptyset\) we can say \(a E b \implies a \in [a]_E, a \in [b]_E \), another contradiction.
...
\subsubsection*{The Union of the Quotient Space}
...
A subset \( f \subseteq X \times Y \) is called a \emph{function} if:
...
\subsubsection*{The Inverse Graph}
...
\subsubsection*{The Composite Graph}
...
\subsubsection*{Functional Graph}

A functional graph is a graph that satisfies the second constraint of functions. Therefore, it is corresponds to a function. Hence, 
all of our graphs properties are applicable to functions.
...
The \emph{image} or \emph{range} of a relation \emph{R} from set \(A\) to set \(B\) is the set 
...
\[
	\mathrm{ran}(R) = \img(R) = \{b \in B \mid \exists a \in A \text{ such that } (a,b) \in R\}
\]

or 

\[
	\img(R[A]) = \{y \in \text{ ran}(R) \mid \exists a \in A \text{ such that } xRy\}
\]
...
\subsubsection*{Proof that the Domain And Range are Sets}
...
Hence, by the definition of the domain and the range of the graph we can directly see that they are subsets of the union of the union of the graph 
...
\subsubsection*{Domain and Range Under Composition Corollary}
...
of our \(y \in \img(H)\) we have can always find a pair \((y, w) \in \img(G)\) due to the structure of our graph. Hence, 
the domain is of our composite graph is \(\dom(H)\).
...
Given  \(T \subseteq M\) a subset of \(M\). The \emph{injection} from \(T\) to \(M\) defined as 

\[
	\iota_{T}: T \to M : t \mapsto t
\]
...
\subsection{Bijection of the Power Set and \texorpdfstring{\(2^A\)}{}}

We denote \(2^A\) as the set of all functions from \(A\) to a set of two elements.
The statement goes as following: there is a bijection between the power set \(\mathcal{P}(A)\) and 
\(2^A\).
...
For the injectivity, we already know that each for \(\gamma(B) = \gamma(C) \implies B = C\) due to 
else being the case that we have to elements in the power set which are identical.
...
The first condition, gives us the injectivity due to the constraint the for all \(b \in B\) there exist exactly one \(a \in A\)
such that \(f(a) = b\). Else, the condition would no be obeyed.

The second one gives us the surjectivity of \(f\) by providing a definition for each of the \(b \in B\). Otherwise, 
...
\subsubsection{Left/Right Inverse}

Given \(f : M \to N\), then  

\begin{itemize}
	
	\item \(f\) is \emph{injective} if there exists a function \(g: N \to M\) with the property \(g \circ f = \text{Id}_M\). Each 
		\(g\) is surjective and called the \emph{left inverse} of \(f\).


	\item \(f\) is \emph{surjective} if there exists a function \(\gamma: N \to M\) with the property \(f \circ \gamma = \text{Id}_N\). Each 
		\(\gamma\) is injective and called the \emph{right inverse} of \(f\).
	
\end{itemize}

\textbf{Example:}

Given \(f: M \to N\) having a right inverse \(\gamma: N \to M\) guarantees the existence of the solution of \(f(m) = n\). Set \(m = \gamma(n)\) then 
\(f(m) = f(\gamma(n)) = f \circ \gamma = \text{Id}_N (n) = n\).

\textbf{Proof I:}

\(\rightarrow\): \(f\) is injective, and we want to proof that from \(g \circ f = \text{Id}_M\) then \(g is surjective\).

Because \(f\) is injective we know that it maps all of \(N\). Suppose, that for some \(n \in N, n \notin f(M)\). We also know that 
\(g \circ f = \text{Id}_M\) by definition. Now with our assumption and this knowledge 

\begin{align*}
	m &= \text{Id}_M \\ 
	  &= g(f(m)) \\ 
	  &= g(n), \text{ for some } n \in N \\ 
	  &= m
\end{align*}

Therefore, such an \(n\) can not exist because of our conditions.

\(\leftarrow\): Because \(g\) is surjective, we know that all \(m \in M\) are mapped and because of 
the composition and its identity property it is clear that \(f\) is injective because \(m = \text{Id}_M (m) \in M\)else the identity property would not hold.
...
Given \(f: A \to B\) with \(|A| = n\) and \(|B| = m\), we can count the number of different types of functions from 
\(A\) to \(B\):

\begin{itemize}

	\item \emph{Total functions:} There are \(m^n\) total functions from \(A\) to \(B\). Each of the \(n\) elements in 
		  \(A\) can be mapped to any of the \(m\) elements in \(B\).

	\item \emph{Injective functions:} If \(n \leq m\), the number of injective functions is given by 
	\(m \times (m-1) \times (m-2) \times \ldots \times (m-n+1)\). This is because the first element in \(A\) has \(m\) choices, the second has \(m-1\) choices, and so on.
	...
	\item \emph{Bijective functions:} If \(n = m\), the number of bijective functions (one-to-one and onto) is 
		  \(n!\) (n factorial), which counts all possible permutations of the \(n\) elements in \(A\) to the \(n\) elements in \(B\).
