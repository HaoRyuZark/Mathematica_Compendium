...
formally defined as a \emph{subset} of the Cartesian product of two sets.
...
A relation is called an \emph{order relation} if it is reflexive, antisymmetric and transitive.
...
A subset \( f \subseteq A \times B \) is called a \emph{function} if:
...
\[
	\mathrm{graph}(f):= \{(x, f(x)) \mid x \in X\} = G
\]

\subsubsection{Basic Properties of Graphs}
