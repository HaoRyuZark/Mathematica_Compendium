\chapter{Graph Theory}
 ...
 In computer science, the density of a graph is a main criterion when modeling a graph. 

 \section{Connected Graphs}

 Let \(G = (V,E)\) be an undirected graph. Then it is called \emph{connected} if, for all pairs \((v_i, v_j) \subseteq V \times V\), there exists a 
 path from \(v_i\) to \(v_j\). We define \(E_S\) as the symmetric relation. 
...
\begin{itemize}
 ...
 For a graph \(G\) with vertices \(V = \{v_1, \dots, v_n\}\), we define the \emph{adjacency matrix} \(A = (a_{ij})\) as a 
...
\section{The Relax Operation}

In graph theory, when referring to the action of updating the cost of visiting a vertex in a weighted graph, we call this operation 
\emph{relaxation}. 

\[
    \operatorname{relax}(u,v, \text{weight}) = 
    \begin{cases}
