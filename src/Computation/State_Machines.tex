...
A \emph{state machine} is a computational model used to design algorithms, systems, or software by breaking them down into a finite number of \emph{states}, 
\emph{transitions}, \emph{inputs}, and \emph{outputs}.  
...
An \emph{NFA} is a state machine defined by a 5-tuple \((Q, \Sigma, \delta, q_0, F)\), where: 
...
\section{Epsilon-NFA \texorpdfstring{(\(\varepsilon\)-NFA)}{}}  

An \(\varepsilon\)-\emph{NFA} is a non-deterministic finite automaton that allows transitions on the empty string 
...
This means the automaton has the following properties:
...
\begin{itemize}
    ...
    \item \emph{Epsilon-Closure:} It is the set of all states reachable via \(\varepsilon\) from a particular 
    state, plus itself. 
...
\section{How to Construct a State Machine}

To construct a state machine, it is necessary that an alphabet and a language are given. Then 
the process revolves around identifying which words are part of the language and how they can be  
expressed as a list of states and transitions.

\textbf{Example:}

Given are the alphabet \(\Sigma = \{a,b,c\}\) and the language \(L = \{ w \in \Sigma^* \mid w = 
...
\section{State Machine Conversions and Minimization}

\subsection{Non-Deterministic Finite Automaton (NFA) to Deterministic Finite Automaton (DFA)}
...
\begin{enumerate}
    ...
    \item We terminate the algorithm when, after evaluating our states, we only get states we have already 
    visited.
...
\subsection{Epsilon-NFA to DFA}

The process is very similar to the previous one. The catch is that we have to keep in mind that 
our initial state is not only \(q_0\) but also all states reachable from \(q_0\) via epsilon.
...
\begin{enumerate}
    ...
    \item Write an empty transition table of the automaton but without the epsilon transition, and then 
          list all epsilon closures of each state
   
    \item For each state and transition, take the \(\varepsilon\)-closure of the state, find where
          these states go with the current transition, and then take their epsilon closure. The final closure
          goes into the corresponding transition column of the current state.
...
Here, every state is a final state, because every state contains the final state in its epsilon closure.
...
\subsection{NFA to Regex}

Converting a regular expression into an NFA is not really difficult, but the other direction is not 
...
\begin{enumerate}
    ...
    Replace set notation with regex syntax: \(\cup \to +\), concatenation is written without an operator 
    ...
    \item Now treat each state as a start state and look for the sets of words this state accepts that lead 
    ...
    \item Repeat step 2 for all \(L_i\), and then replace the set of words with regular expressions and the 
    ...
    \item Start substituting equations into other equations. Here there are two cases: either the  
          ...
          doing the substitution until we have only one equation, which is going to be our regex; but for the 
...
Given \(X, Y\) as formal languages with \(\varepsilon \notin Y\), the following holds:

\[
    L = YL \cup X \text{ has exactly one solution: } L = Y^*X
\]

And for the special case \(X = \emptyset\):
...
To show that \(L = Y^*X\) is a solution and, more specifically, the only solution.
...
Then there is the shortest word \(w\) in \(L\) which is not in \(Y^*X\). But 
because \(L = YL \cup X\), \(w\) must be either a word in \(X\), which is not possible because 
\(w \notin Y^*X\), or in \(YL\). This implies that \(w\) can be decomposed into \(uv\) with 
\(u \in Y, u \neq \varepsilon, v \in L\). But this is a contradiction, since then \(w\) would be a word of the 
form \(Y^*X\), which is not true, and this would imply that there is an even shorter word than the shortest word, 
and so on. Therefore 
...
\section{Minimizing State Machines}
...
\subsection{Table Method for DFA Minimization}

Steps:

\begin{enumerate}
    
    \item Put all non-accepting states in an equivalence class \(\alpha\), and the accepting states in another 
          called \(\Omega\). Write a table with the following columns: state, 
          transition 1, transition 2, ..., transition n. On the first row put \(\alpha\) and in the state column 
    ...
    \item Terminate the process if no more new class tuples appear.
