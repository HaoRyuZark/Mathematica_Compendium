\chapter{State Machines and Languages}

\section{Languages and Alphabets}

A \emph{language} \(L\) is a subset of words of an \emph{alphabet} \(\Sigma\), which is a finite, 
non-empty set of symbols that always includes the \(\varepsilon\)-word, the empty word.

\subsection{Concatenation}

\emph{Concatenation} is an operation that can be performed with members of an alphabet, and it is like 
basic addition. Therefore, it is associative, and when \(\varepsilon\), the zero of alphabets, is added, it does not affect the words 
or their length.
...
Define a function that counts the occurrences of \(ue\) in a word with letters of the alphabet 
\(\Sigma = \{a, \dots, z\}\).
...
\emph{Head} returns the first symbol of a word and is defined by 
...
\emph{Tail} removes the first symbol of a word and is defined by 
...
\(\varepsilon\) is a prefix, suffix, and substring of all words. 

\section{The Canonical Order}
...
Other parts of a language are the \emph{syntax}, which defines the words in the language, and the \emph{semantics}, which determines 
...
\subsection{Pumping Lemma Algorithm}

Given two alphabets \(\Sigma_1, \Sigma_2\) and \(\mathcal{D} \subseteq \Sigma_{1}^{*}\), an \emph{algorithm} is a total function 
...
The \emph{decision problem} \(\inner{\Sigma}{L}\) for a given alphabet \(\Sigma\) and a language \(L \subseteq \Sigma^{*}\) is to decide, for each \(w \in \Sigma^{*}\), whether  
...
Therefore, one also speaks of the \emph{word problem} \(\inner{\Sigma}{L}\).  

An algorithm \(A \colon \Sigma^{*} \to \{0, 1\}\) \emph{solves} the decision problem \(\inner{\Sigma}{L}\) if, for all \(w \in \Sigma^{*}\), we have
...
\section{Recursive Languages}

Let \(\Sigma\) be an alphabet and let \(L \subseteq \Sigma^{*}\) be a language.  
\(L\) is called \emph{recursive} if and only if there exists an algorithm \(A\) that solves the decision problem \(\inner{\Sigma}{L}\).  

\section{Enumeration Problem}

Let \(\Sigma\) be an alphabet, \(L \subseteq \Sigma^{*}\) a language, and \(n \in \mathbb{N}\).  
The \emph{enumeration problem} \(\inner{\Sigma}{L, n}\) for \(L\) consists of listing the canonical first \(n\) words of \(L\).  
...
Thus, a \emph{grammar} \( G \) defines a \emph{formal language} \( L(G) \) as
