...
\section{Leibniz Inference Rule}

To clarify, the notation we will be using from now on looks like a fraction, but it means that assuming 
the statement above is true, then the following (below) is also true. 
...
The \emph{correctness} of an algorithm is determined by the sum of the following properties: 
the \emph{partial correctness}, which means the algorithm at each step does the right thing, and 
\emph{termination}, which, like the name, means that the algorithm terminates. 

\subsection{Partial Correctness}

If before running \(P\) the condition \(\varphi\) is true, then after the termination of \(P\), 
\(\psi\) should be true; these are the pre- and post-conditions.
...
\subsection{Hoare Calculus}

To read the following expressions, which look like fractions but are not, use the following:
...
If \textbf{x} is a variable and \textbf{t} is an expression without side effects, then \(\varphi \text{x/t}\) is 
...
Note that this rule only has the part below, which means that this rule has no precondition.
...
This lets us make the post-condition weaker.
...
Note that two guards can only be consecutive if the one below is implied by the one above. 
...
This rule enforces that, no matter which branch of the program is taken, the post-condition 
is true.
...
In this rule, we look for some \emph{invariant} \(\phi\) which is true before and after the loop.
...
Finding the invariant can be tricky, but it commonly involves finding an integer expression, or some formula which 
