...
An important note: depending on the type of random variable, discrete or continuous, the formulas use the logarithm base 2 or the natural logarithm.
...
``surprising'', while an improbable event is.
...
For the intuition, given an event \(y\), what is our average surprise under such conditions?
...
If one of the random variables is continuous, then we can make bins for different ranges of values and then proceed 
...
It measures surprise from using \(Q\) to approximate \(P\). It is the reverse of the cross entropy, where we use 
\(P\) to approximate \(Q\).
...
Subset \(S_{v_1}\) with 4 samples (3 from class A and 1 from class B) and subset \(S_{v_2}\) with 6 samples (3 from class A and 3 from class B).
...
\emph{Gini Impurity} is a measure of how often a randomly chosen element from the set would be incorrectly labeled if it were randomly 
