...
the permutations. It is chosen that way because for each choice of \(k\) objects there are \(k!\) possible orderings, and we only 
want one.
...
the number of occurrences of the object \(i\), then we can imagine using \(\mid\) a number of times for the occurrences of each object 
and \(\cdot\) as the separator between each set of \(\mid\). For each possible combination we have \(k\) lines, \(k + n - 1\) 
