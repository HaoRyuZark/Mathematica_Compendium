...
NThe \emph{permutation with repetition} counts theumber of ways of ordering \(n\) non-distinct elements. Here \(m_1, \dots, m_n\)
...
The \emph{combination} can be unsertood as choose \(k\) objects of \(n\) elements. Or put \(k\) elements from \(n\) into \(k\) slots where the order does not matter.
...
A \emph{disarray} is the number of permutation in which no object ends in the same initial spot. Also, know as the 
\emph{sub-factorial}.
...
The \emph{Ramanujan numbers} are same as the Bell Numbers but, all objects are equal. As an example the number of ways
...
The \emph{Lah numbers} represen the ways of building \(K\) lists with a set \(N\) elements.
