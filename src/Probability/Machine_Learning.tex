...
\[
    \theta(s(x)) = \theta \left(\langle w, x \rangle\right)
\]
...
Our hypothesis set \(\mathcal{H}\) is composed of hypotheses of the form \(h(x) = \langle x, w\rangle, x_0  = 1\).
...
\(\phi: \mathcal{X} \to \mathcal{Z}\) is a non-linear transformation. We also define \(\hat{g}(z) = \text{sign}(\langle \hat{w}, z \rangle)\).
...
It maps data points into an infinite-dimensional space. The RBF kernel is particularly effective in handling non-linear 
relationships in data.

The way it works is by measuring the similarity between two data points based on their distance. 
Points that are closer together in the input space will have a higher similarity score, while points that are farther apart will have a 
lower score.

The parameter \(\sigma\) controls the width of the Gaussian function. A smaller \(\sigma\) leads to a narrower kernel, meaning that only 
points very close to each other will have a significant similarity score. Conversely, a larger \(\sigma\) results in a wider kernel, 
allowing points that are farther apart to have a higher similarity score. The parameter is optimized via cross-validation. 
