...
Thus, for \(\mu \ne \vec{0} \in \Reals^n\) and \(\Sigma \in \Reals^{n \times n}\) positive definite,
...
Given \(\mu \in \Reals^{n}\) and \(A \in \Reals^{n \times n}\), if \(Z \sim \mathcal{N}(\vec{0}, I_n)\), then
...
For random variables \(X_1, \dots, X_n\) (not necessarily independent) with \(\Expc[X_i] = \mu_i\) and constants \(a_1, \dots, a_n, b \in \Reals\), the expected value of a linear combination is always:
...
\subsection{Detailed Example: Spare Parts Delivery}
...
Given \(X_1, \dots, X_n\) random variables with a joint normal distribution \(\mathcal{N}(\mu, \Sigma)\). For any sequence of coefficients \(a_1, \dots, a_n \in \Reals\), the linear combination \(\sum_{i = 1}^{n} a_i X_i\) is also normally distributed.

Specifically, if \(X \sim \mathcal{N}(\mu, \Sigma)\) and \(a \in \Reals^n\), then:
\[
