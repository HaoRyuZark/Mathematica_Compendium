...
\subsection{Joint Probability of An Event}
...
\subsubsection{Marginal Probability Function for Multiple Discrete Variables}
...
\subsection{Multivariate Expected Value for Discrete Random Variables}
...
\subsubsection*{Expected Value as a Special Case of the Transformation Formula}
...
\subsection{Multivariate Expected Value for Continuous Random Variables}
...
Let \( u : \R^n \to \R \) be a measurable function.
Then, if the expectation exists, the following holds:
...
Let \( I \subseteq \R^n \) be a measurable set.
Random variables \( (X_1, \ldots, X_n) \in \R^n \) with joint density
...
Let \( X_1, \ldots, X_n \) be random variables with joint density \( f(x_1, \ldots, x_n) \),
and let \( u : \R^n \to \R \) be a measurable function.
...
\[
    f(x_1, \dots ,x_n) = \prod_{i = 1}^{n} \frac{1}{\sqrt{2\pi}} e^{\left(-\frac{1}{2}x_{i}^{2}\right)} = \frac{1}{(2\pi)^{n/2}} e^{\left(-\frac{1}{2} X^T \cdot X\right)} \sim \mathcal{N}(\vec{0}, I_n)
\]
...
\subsubsection{Computing Expected Values and Variances of Linear Combinations}

\paragraph{Expected Value of Linear Combinations}
For random variables \(X_1, \dots, X_n\) (not necessarily independent) with \(\expc[X_i] = \mu_i\) and constants \(a_1, \dots, a_n, b \in \R\), the expected value of a linear combination is always:
...
\paragraph{Variance of Linear Combinations}
...
\textbf{Important:} This equivalence between zero correlation and independence is special to the multivariate normal distribution and does not hold in general.

\subsubsection{Linear Combinations of the Joint Normal Distribution}
Given \(X_1, \dots, X_n\) random variables with a joint normal distribution \(\mathcal{N}(\mu, \Sigma)\). For any sequence of coefficients \(a_1, \dots, a_n \in \R\), the linear combination \(\sum_{i = 1}^{n} a_i X_i\) is also normally distributed.
