...
Let \(f \in \R^n \to \R,\ \tilde{x} \in \R^n\), then \(\tilde{x}\) is a \emph{global minimum} if
...
\begin{itemize}
    ...
    This works because the second derivative tells us about the concavity of the function. If it is 
    positive, then we will have a function which is concave up, a minimum; and if negative, concave down, a maximum. 
...
\begin{enumerate}
    ...
    \item Find critical points by solving:
        ...
        This must hold for each of the equations. This process is called \emph{finding the candidates}, which we denote as \(\tilde{x}\).
...
extreme points. However, if the Hessian shows definite behavior via eigenvectors, we can guarantee that our problem is a convex optimization problem.
...
or directional behavior, i.e., taking the limit at that point when analyzing the derivatives.
...
normal to them. We also are going to see that both the gradient of \(f\) and the gradient of \(g\) are just scalar versions of each other. 

We start with the \emph{Lagrange system}:
...
We can also compact these equations in the \emph{Lagrangian} as
...
In certain cases, this is the way to solve a problem of this topic, hence, here are the steps to follow.
...
The Bordered Hessian matrix is defined as
...
The function \(f(x,y)\) is in this case the Euclidean norm:
...
In this case we can use a property of the root function to our advantage. Notice that the minimum 
of the root function is the minimum of the term under the root; therefore, we can simplify some work by writing 
...
Now, this is the difficult part. The best approach in this case is to use substitution 
...
just find the minimum of the part under the root; thus, \(f(x,y,z) = x^2 + y^2 + z^2\). Now we can 
...
\(\lambda = -1\) or \(y = 0\). Thus, we have our first numbers to work with.
...
A function \(f\) is \emph{convex} if \(\metric{x - y} \in X, \ \forall x, y \in X\), where \(X\) is the \emph{epigraph} of the function. Visually, 
in 2D and 3D, it is cup-shaped. 
...
this problem can be easier to solve than the primal problem, and using the Karush-Kuhn-Tucker conditions, we get 
