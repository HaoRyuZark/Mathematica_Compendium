...
\textbf{Natural Join = Cartesian Product + Selection + Projection}
...
Returns tuples from one relation that \emph{have a match} in the other.
...
Formally \(alpha \to \beta\), \(\beta\) is dependent on \(alpha\) if

\[
    \forall \, r(\mathcal{R}), \forall d_1, d_2 \in r (d_1[\alpha] = d_2[\alpha] \implies d_1[\beta] = d_2[\beta])
\]

where \(r(\mathcal{R})\) is a relation in the Schema \((\mathcal{R}, \tau)\) and \(\alpha\)
is a tuple in the schema.
...
The \emph{synthesis algorithm} is used to convert relations to the 3NF.
...
\emph{Test for key candidates}

No relation has the key-candidate \(AC\).

\subsection{Decomposition Algorithm for BCNF}

The \emph{decomposition algorithm} is used to convert relations to the BCNF. 
...
\begin{itemize}
    ...
    \item As long as there schemata \(R_i \in Z\) gives which are not in BCNF proceed to 
        ...
        \subitem Split \(R_i\) into \(R_{i1} = \alpha \cup \beta\) and \(R_{i2} = R_i - \beta\).

        \subitem Remove \(R_i\) from \(Z\) and add \(R_{i1}\) and \(R_{i2}\).
