...
Given \(n \in \Naturals\). We define the relation \(\sim_n\) for \(\Integers\).
...
\subsection{Equivalence Class of \(\Integers\)} 

For \(i, n \in \Integers\) 

\[
    \overline{i} := i + n \Integers = \{ i + na \mid a \in \Integers\}
\]

and 

\[
    \Integers_N = \Integers / n \Integers = \{\overline{0}:= n \Integers, \overline{1}:= 1 + n \Integers, \dots, \overline{n - 1} := n - 1 + n \Integers \}    
\]

is the set of all equivalence relationships for \(\Integers\) with respect to the relation 
\(k \sim l \iff n \mid (k - l)\). We define the addition and multiplication for \(\Integers / n \Integers\) using the 
representatives 

\[
    (a  + n \Integers) + (b + n \Integers) := (a + b) + n\Integers
\]

\[
    (a  + n \Integers) \cdot (b + n \Integers) := ab + n\Integers
\]

For all \(a, b \in \Integers\). Therefore, 
