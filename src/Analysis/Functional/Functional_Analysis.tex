...
\(\Phi\) is considered a \emph{contraction} according to \(\metric{\cdot}\) on \(X \subseteq \R^n\) if 

\[
    \Phi: X \to X
\]

and there is an \(\alpha < 1\) independent of \(x,y\) such that 

\[
    \metric{\Phi(x) - \Phi(y)} \le \alpha \metric{x - y} \ \forall x,y \in X.
\]

To check whether a function is a contraction on a certain set or interval, we need to check the following properties:

\begin{itemize}

    \item Check that \(f:X \to X\), i.e., that the function at its local extremes and endpoints remains in the interval. This is 
    ...
    \item Estimate \(\alpha\) using the definition of the contraction and approximations \(\ge\). Alternatively, we can use the \emph{Mean Value Theorem} with 
...
Since \(f(x)\) is monotone, we just need to check the function at the endpoints of the interval.
...
such that \(\alpha < 1\) is independent. Then \(\Phi\) has exactly one \emph{fix-point} \(x = \Phi(x)\) in \(X\). Our
\emph{Picard iteration} converges for all \(x_0 \in X\) towards \(x\). Additionally, the following approximations hold:
...
The iteration is linearly convergent if such a constant \(c\) exists.
...
linearly convergent, \(0 \le c\) is independent of \(k\), and 
...
converges for all \(x_0 \in X\) with at least order \(m\).
