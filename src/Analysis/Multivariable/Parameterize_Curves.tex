...
\( \R^n \) is a continuous function:

\[
    \mathbf{r}(t) = (x(t), y(t), z(t), \ldots),
\]

where \(t\) ranges over some interval \( I \subseteq \R \), and each component function 
(e.g., \( x(t) \), \( y(t) \)) is real-valued.

\subsection{Purpose}

The \emph{purpose} of parameterizing a curve is to provide a flexible and detailed way of describing geometric 
...
\section{From Parametric to Explicit Form}

This process is rather simple because, in the case where it is possible to change from one form to the other, we only need to 
write the parameter in terms of one of the variables and then substitute.

\subsection{How to Parameterize a Function}

To \emph{parameterize} a curve or a function:
...
\section{Arclength and t Parameter}

Recall that the \emph{arclength} of a curve in 3 dimensions is

\[
    \int_{a}^{b} \sqrt{{f'(t)}^2 + {g'(t)}^2 + {h'(t)}^2} \, dt,
\]

which is the norm of our tangent vector,

\[
    \int_{a}^{b} \sqrt{|\vec{v}(t)|} \, dt.
\]

\subsection{Arclength Parameter}
...
This parameter is intrinsic to the curve and represents one choice, because there is only one valid
interpretation of the length. The downside is that this is hard to compute.

\subsection{The t Parameter}

For the \(t\) parameter, we can say that it is intrinsic to the object along the curve. It can be
interpreted in multiple ways and therefore offers many choices, and it is easy to compute.
