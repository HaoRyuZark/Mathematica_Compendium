...
Let \( \metric{\cdot} \) be a norm on \( \R^n \). The \( \varepsilon \)-neighborhood of a 
point \( \vec{x}_0 \in \R^n \) is defined as:

\[
    U_\varepsilon(\vec{x}_0) := \left\{ \vec{x} \in \R^n \mid \metric{\vec{x} - \vec{x}_0} 
    < \varepsilon \right\}
\]
...
Let \( (\vec{x}_n) \subseteq \R^n \) be a \emph{sequence} of vectors. We say that 
\( \vec{x}_n \to \vec{x} \in \R^n \) as \( n \to \infty \) if for every \( \varepsilon > 0 \), 
there exists \( n_0 \in \N \) such that:
    
\[
    \metric{\vec{x}_n - \vec{x}} < \varepsilon \quad \text{for all } n > n_0
\]
...
From linear algebra, it is known what a \emph{Norm} is, along with its properties:

Let \( V \) be a vector space over \( \R \) or \( \C \). A function 
\( \metric{\cdot}: V \to \R \) is called a norm if for all \( x, y \in V \) and all scalars 
\( \alpha \), the following properties hold:

\begin{enumerate}
    
    \item \emph{Non-negativity:} \( \metric{x} \geq 0 \), and \( \metric{x} = 0 \) if and only if \( x = 0 \).
    
    \item \emph{Absolute homogeneity:} \( \metric{\alpha x} = |\alpha| \cdot \metric{x} \).
    
    \item \emph{Triangle inequality:} \( \metric{x + y} \leq \metric{x} + \metric{y} \).
...
\begin{itemize}
    ...
    \[
        \metric{x}_1 = \sum_{i=1}^n |x_i|
    \]
    
    \item \emph{Max Norm} (also known as the \( \ell_\infty \) norm): 
    
    \[
        \metric{x}_\infty = \max_{1 \leq i \leq n} |x_i|
    \]
    
    \item \emph{Euclidean Norm} (also known as the \( \ell_2 \) norm): 
    
    \[
        \metric{x}_2 = \left( \sum_{i=1}^n |x_i|^2 \right)^{1/2}
    \]
...
Now use the definition of convergence and the \emph{Euclidean Norm}:

\[
    \metric{\vec{X}_n - \vec{X}_0} < \varepsilon
\]
...
In finite-dimensional vector spaces, all norms are equivalent. That is, if \( \metric{\cdot}_a \) and 
\( \metric{\cdot}_b \) are two norms on a finite-dimensional vector space \( V \), then there exist constants 
\( c, C > 0 \) such that for all \( x \in V \),

\[
    c \metric{x}_a \leq \metric{x}_b \leq C \metric{x}_a
\]
...
\subsection{Equivalence in the Reals}

In \(\R^n\), all norms are equivalent, and

\begin{align*}
     \metric{\vec{x}}_\infty &\le \metric{\vec{x}}_2 \le \sqrt{n}\metric{\vec{x}}_\infty \\
     \metric{\vec{x}}_\infty &\le \metric{\vec{x}}_1 \le n\metric{\vec{x}}_\infty \\
     \metric{\vec{x}}_2 &\le \metric{\vec{x}}_1 \le n\metric{\vec{x}}_2
\end{align*}
...
A function \( f : U \subseteq \R^n \to \R \) has the \emph{limit} \( g \in \R \) at a 
...
To check if a multi-variable function is continuous, it is enough to prove that it is continuous at 
the origin:
...
If we are checking whether a function is continuous and get an \emph{indeterminate form}, there are three main 
approaches.
...
\begin{enumerate}
    ...
    \item \emph{Different Paths}: Trying different paths can be pretty straightforward. If we get two 
...
\subsection{Continuity in Polar Coordinates for 2D}
...
Because of the dependence on \(\theta\), which for \(\theta = \frac{\pi}{4}\) gives us \(\frac{1}{2}\), 
...
\section{Epsilon–Delta Criterion}
...
This generalizes the familiar \emph{epsilon-delta criterion} from single-variable calculus to multi-variable 
...
A \emph{vector field} is a function \(\vec{f}: \R^n \to \R^n\); in other words, simply a 
...
As a side note, the \emph{gradient} of a multi-variable function is a vector field.
