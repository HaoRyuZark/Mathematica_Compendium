...
We call a function \(f\) \emph{differentiable} at \((x_1, x_2, \cdots, x_n)\) if and only if:
...
The angle of the maximal slope is \(\phi = 0\). This comes from the fact that 
the directional derivative, which is defined as
...
\[
    \langle \nabla f, \vec{v}\rangle = \metric{\nabla f} \metric{\vec{v}} \cos(\phi)
\]

tells us that our gradient is orthogonal to the tangent vector of the level curve, i.e., \(\phi = \frac{\pi}{2}\), and 
the largest slope is when \(\phi = 0\), which is the direction of the gradient.

Another way of thinking about this fact is using the definition as a projection of the dot product. 

\[
    \mathrm{proj}_{\nabla f} (\vec{v}) = \frac{\langle \nabla f, \vec{v}\rangle}{\metric{\nabla f}^2} \nabla f
\]

Now our directional derivative is the projection of the gradient onto the direction vector \(\vec{v}\), and we want to maximize this projection.
This happens when \(\vec{v}\) is in the same direction as the gradient, i.e., \(\vec{v} = \nabla f\). Hence, the direction of the maximal slope is 
in the direction of the gradient.

Formally, we have a vector \(d^*\) that points in the direction of the maximum slope, defined by

\[
    d^* = \text{arg}_{\metric{d} = 1} \text{max} D_{\vec{v}}f(\vec{x})
\]
...
\[
    d^* = \text{arg}_{\metric{d} = 1} \text{max} \langle \nabla f, \vec{v} \rangle 
\]

Then, using the definition of the dot product,

\[
    d^* = \text{arg}_{\metric{d} = 1} \text{max} \metric{\nabla f} \metric{\vec{v}} \cos(\phi) 
\]
...
\section{Origin of the Formula}

Suppose along the curve \(r(t) = x_1(t)\hat{v_1} + x_2(t)\hat{v_2} + \cdots + x_n(t)\hat{v_n}\) that 
\(f(x_1(t), \dots, x_n(t)) = C\).

For a fixed \(t\), we get a constant value \(C\), and differentiating with respect to \(t\) gives us the following:
...
We can write it as a dot product of two vectors:
...
From the formula, we see that \(\nabla f\) is the normal and \(\frac{d\vec{r}}{dt}\) is the tangent vector 
of the curve.
...
The \(\frac{\operatorname{grad}f}{\metric{\operatorname{grad}f}}\) points in the direction of the 
steepest increase of \(f\), and \(- \frac{\operatorname{grad}f}{\metric{\operatorname{grad}f}}\) in 
the direction of the steepest decrease.

\section{The Tangent Plane}

The \emph{tangent plane} to a differentiable surface \( z = f(x, y) \) at the point \( (x_0, y_0) \) is the 
...
and let \( \vec{v} \in \R^n \) be a direction vector with \( \metric{\vec{v}} = 1 \). The 
...
If \(f\) is differentiable at \( \vec{x}_0 \), then the directional derivative can be computed using the gradient:
...
\[
    |\langle \vec{a}, \vec{b} \rangle| \le \metric{\vec{a}} \cdot \metric{\vec{b}}
\]

This implies:

\[
    |D_{\vec{v}}f(\vec{x}_0)| \le \metric{\nabla f(\vec{x}_0)}
\]

\subsection{Derivation of the Directional Derivative Formula}
...
This is the \emph{chain rule} in vector form.

Or, to be more explicit, a small change in \(t\) induces a change in \(x_1, x_2, \cdots\), and all these changes 
...
The calculated length is \( z = 5 \, \text{cm} \). We now determine the maximum possible absolute 
error in \(z\).
...
If the final result \(z\) must be accurate to within \( \varepsilon_z = 0{,}01 \) (i.e., \(1\%\)), then we must 
satisfy:
...
If a function \( F(x, y) = 0 \) defines \(y\) \emph{implicitly} as a function of \(x\), and \(F\) is 
differentiable, then:
...
Given \(x^2 + y^2 + z^2 = 1\), differentiate with respect to \(x\). 
Regard \(z\), the target function, as a function of the other variables, 
in this case \(z(x,y)\).
...
with respect to \(x\) at the point \( (1, 1) \).
...
\section{Schwarz’s Theorem (Clairaut’s Theorem)}
...
For a function \(f:\R^n \to \R\),  we define the \emph{Hessian} as the matrix composed of its second derivatives.
...
When analyzing the Hessian at critical points, we get:

\begin{itemize}
        
    \item For the \(\R^2\) case we use:

        \subitem If \( H_f(x)\) is \emph{positive definite} \(f_{xx}(x_0, y_0) > 0,\, \det(H) > 0\), then \((x_0, y_0)\) is a local minimum.

        \subitem If \( H_f(x)\) is \emph{negative definite} \(f_{xx}(x_0, y_0) < 0,\, \det(H) > 0 \), then it is a local maximum.
    ...
    \item For the general case \(\R^n\), we use the eigenvalues:
...
the \emph{Taylor polynomial} of degree 2 around \( \vec{x}_0 \) is:
...
\subsection{Derivation of the Tangent Vector Formula}
