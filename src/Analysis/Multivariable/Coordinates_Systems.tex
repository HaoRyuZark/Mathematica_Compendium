...
\emph{Coordinate systems} provide different ways of describing points in space, adapted to the geometry of the 
problem. Below are the most common systems used in multi-variable calculus.

\section{Cartesian Coordinates}

The \emph{Cartesian coordinate system} uses mutually orthogonal axes. A point in \( \R^2 \) 
is given by:

\[
    (x, y) \quad \text{and in } \R^3: \quad (x, y, z).
\]
...
In \emph{polar coordinates}, a point \( (x, y) \in \R^2 \) is described by:

\[
    r \ge 0 \text{ (radius from origin)}, \quad \varphi \in [0, 2\pi) \text{ (angle from the \(x\)-axis)}
\]
...
\emph{Cylindrical coordinates} extend polar coordinates by adding a height \(z\) component:

\[
    (r, \varphi, z) \quad \text{where } r \ge 0, \ \varphi \in [0, 2\pi), \ z \in \R
\]
...
In \emph{spherical coordinates}, a point \( (x, y, z) \in \R^3 \) is represented using:
