...
A Differential Equation is an equation that relates a function (the dependent variable) with a variable or multiple variables (the independent variables) 
and its derivative.

They can be categorized into \emph{Ordinary} and \emph{Partial} Differential Equations.
...
is called an \emph{Ordinary Explicit Differential Equation of order} \(n\).

If \(y^n = 0\), then it is \emph{implicit}.

If the exponent of the dependent variable is 1, and it is built like a linear equation, then it is categorized as \emph{Linear}.

\section{Initial Value Problems and Solutions}
...
An \(n\)-times differentiable function \( y(x) \) that satisfies the explicit differential equation is called the \emph{general solution} of the
differential equation. If it also satisfies the conditions of the initial value problem from the previous definition, it is called the 
\emph{particular solution} of the IVP\@.

\section{Volterra Integral}
...
\section{The Picard-Lindelöf Iteration Method}
...
\section{Integration Case}

To solve an ODE of the form \(y' = f(x)\), it is as easy as integrating \(f(x)\).

\section{Separation of Variables}

An equation \(F(x,y,y',\dots) = 0\) is called an equation of \emph{separable variables} if it can
...
\subsection{Steps to Solve an ODE of Separable Variables}
...
It is an ODE of separable variables.
...
\subsection{Understanding the Fiction}

The previous fiction we used can be explained in the following manner.
...
\section{Geometry of ODE}

You can imagine the plot of an ODE as a \emph{Slope field} where the slope (derivative) of a function
obeys a certain condition, namely the differential equation \(\frac{dy}{dx} = expr\).

A point on this field is called an \emph{Initial Condition}, and the curve that goes through
that point in the slope field is called an \emph{Integral curve}, which is a solution to the differential
equation with respect to that initial condition.

\section{Existence and Uniqueness}

Let \( f(x, y) \) be continuous on a rectangle \( [a, b] \times \R \). Then the differential
equation
...
A function \( f : \R^2 \to \R \) is called
...
Let \( x_0 \in [a, b] \), and let \( f : [a, b] \times \R \to \R \) be continuous
...
\section{How to Deal with Initial Conditions}
...
in our general solution with respect to our initial conditions. For example, given \(y(0) = 1\),
we would plug in 0 for all \(x\)'s in our general solution and set the whole equation equal to 1.

\section{Solving DE with Substitution}
...
For this to be valid with \(z(x) = \frac{y}{x}\), our \(y(x)\) needs to look like \(y(x) = z(x)x\)
and so \(y' = z(x) + z'(x)x\).
...
An ODE is called linear if all occurrences of the dependent variable and itsderivatives have an exponent of at most 1.

\[
    a_n(x)y^n + a_{n - 1}y^{n -1} + \cdots + a_1(x)y' + a_0(x)y = b(x)
\]

If such an equation also has the property \(b(x) = 0\), it is called \emph{Homogeneous}.

\section{How to Solve a Linear Differential Equation}

In this case, we will focus on first-order ODEs.

Write it in the standard form \(y' + p(x)y = f(x)\).

Now use the \emph{Integrating Factor Method}.

\section{Integrating Factor Method}

We are going to multiply our expression by a function \(r(x)\)

\[
    r(x)y' + r(x)p(x)y = r(x)f(x)
\]

Now we would like to write the left side as \(\frac{d}{dx} y r(x)\) because, if we
were to integrate, it would take us to the expression on the right.
...
Now we have found an \(r(x)\), and now that we know such a function exists we can return
...
To find our specific solution, we just plug 0 into our general solution, which gives us
...
The equation \(y' + f(x)y = 0\) is called a \emph{homogeneous linear differential equation of first order}.

\subsection{Homogeneous Equation}

A function \(f(x,y)\) is called \emph{homogeneous} if and only if it can be written as
...
It is important to note that the key is that each term has a total degree of \(n\).
For example: \(x^2y + x3y^2\)

\subsection{Homogeneous Functions Theorem}
...
\begin{enumerate}
    ...
    \item Solve using the method of separable variables.
...
Given is the function \((x - y)dx = -x\,dy\)
...
\textbf{Step 5.} Now we use \(u = \frac{y}{x}\) to return to our original variables
...
\subsection{Solving Homogeneous DE II}

Another method for solving this kind of equation \(y' + f(x)y = 0\) is to follow the next steps.
...
The equation \(y' + f(x)y = g(x)\) is called a \emph{linear inhomogeneous differential equation
    of first order}; \(g(x)\) is called the \emph{distortion function}. The corresponding IVP is called
a \emph{linear} IVP\@.

\section{Solutions of LDE}
...
\section{Variation of the Constants}

Recall that for the homogeneous case \(y' + f(x)y = 0\) we use
...
\section{Existence and Uniqueness of Differential Equations}
...
\section{Theorem of Superposition and the General Solution}
...
partial solution \(y_p\). Now remember from \emph{Linear Algebra} that
\(general = particular + homogeneous\), more precisely \(y = y_p + y_h\); therefore, adding a
particular solution to a homogeneous one does not change
...
\section{Steps for Solving Constant Coefficients Problems}
...
\section{Find the Partial Solution}

We will use a method based on guessing the type of the function on the right side.
...
thus, we can substitute \(y = bx + c\) and differentiate to get \(y' = b\). In the case when we have a higher
...
The best way is to tackle each of the types of functions separately.

An alternative way of using this method is to
...
We cannot use the methods we already know for this equation because it is not linear, but with a
...
Now this
looks kind of similar to the original equation; multiply by \(y^{-n}\), which looks like
...
Now we can use the \emph{Integrating Factor Method}. First bring it to the standard form.
\(y' + p(x)y = f(x)\)
...
Now let us use \(y_p = \frac{c(x)}{x}\).

\[
    y_{p}' = \frac{c'(x)x - c(x)}{x^2}
\]

Let us break down the fraction to find where to put the homogeneous solution we have already found
...
is to find the function given initial conditions. This can be better understood with an example.
...
Now let us differentiate our \(y\)
...
more specifically, after dividing by \(A(x)\)
...
The radius of convergence is at least as large as the distance to the nearest \emph{Singular Point}.
...
We define this kind of differential equation as \emph{exact}. And
\(P_y = Q_x\) is called the \emph{Integrability-Condition}.

By finding the \emph{potential}, we can solve this kind of differential equation.
...
\section{Euler Multipliers}

Sometimes a seemingly \emph{exact} differential equation is not possible to solve due to the partial
derivatives of \(P(x,y)\) and \(Q(x,y)\) not being equal. An approach to this problem is the so-called
\emph{Euler Multipliers}, which are factors \(\mu(x) \text{ or } \mu(y)\) that, when found, can
...
The steps for solving this kind of problem are
...
\begin{enumerate}
    ...
    \item Set both partials equal and solve for \(\mu\) in the newly created differential equation
          using the necessary method. Also make sure that the solution has no zeroes. This is necessary
          because otherwise we would have multiplied our original equation by zero.

    \item Substitute \(\mu\) with the solution. If both partials are now equal, then you can
          solve the exact differential equation. If not, try with another \(\mu\) using the other variable.
          And if this also does not work, then the differential equation cannot be solved. More specifically,
...
\subsection{Case 1: Two Distinct Real Roots \texorpdfstring{\( (D > 0) \)}{}}

There is not much to do here; we just plug in the solutions of the root.

\[
    y = c_1 e^{r_1 t} + c_2 e^{r_2 t}
\]

\subsection{Case 2: Repeated Real Root \texorpdfstring{\( (D = 0) \)}{}}
...
Note that this could be counterintuitive, but \(te^{rt}\) is a solution and is linearly independent.
For more repeated roots, you can use higher powers of \(t\): \(t, t^2, t^3, \dots\).

\subsection{Case 3: Complex Conjugate Roots \texorpdfstring{\( (D < 0) \)}{}}
...
For the inhomogeneous case, we apply the method of \emph{Undetermined Coefficients},
differentiating two or more times depending on the order of the differential equation, and then add the
homogeneous solution to the particular solution we obtain.
...
The homogeneous equation has a solution consisting of the linearly independent \(y_1\) and \(y_2\),
...
Now we can differentiate one more time:
...
With non-generic functions, a lot of the terms would cancel out. After that, we multiply one of the rows
by \(y_1\) and the other by \(y_2\) so that we can cancel them in a row operation. This will leave us
with only two integrals to solve.

\section{Which Method to Use Depending on the Type of Differential Equations}
...
\subsection{Second Order and Higher}
