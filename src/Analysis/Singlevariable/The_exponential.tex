\chapter{The Exponential Function and Relatives}

The \emph{exponential function} and its related functions, such as sine, cosine, and the hyperbolic functions, 
are fundamental in both pure and applied mathematics. This chapter explores these functions through their 
...
\section{Exponential Function as a Series}

The \emph{exponential function} \(e^x\) is defined as an infinite series:
...
\begin{itemize}
    ...
    \item \(\exp(x) < \exp(y), \forall x, y \in \R : x < y\)

    \item \(\exp(x) \ge 1 + x, \forall 1 + x\)

    \item \(|\exp(x) - 1| \le \frac{|x|}{1 - |x|}\) for all \(x \in \R\) such that \(|x| < 1\)

    \item For a large enough \(x\): \(\exp(x) > x^n, \forall n \in \N\) and \(x > 4n^2\)
...
\section{Difference of Exponentials}
...
\section{Sine and Cosine Definitions with \texorpdfstring{\(e\)}{e}}

Using Euler's identities and Taylor series, \emph{sine and cosine} can be defined in terms of exponential 
functions:
...
One of the most elegant formulas in mathematics is \emph{Euler's formula}:
...
\section{Reason for the Infinite Series of \texorpdfstring{\(e\)}{e} (Differentiation)}
...
\section{Hyperbolic Functions with \texorpdfstring{\(e\)}{e}}

The \emph{hyperbolic functions} are analogs of the trigonometric functions but defined using the exponential 
function:
