...
\subsection{Boundness of Cauchy Sequence}
...
\(a_n\) is bounded because else \( |a_n - a_m| > \varepsilon \) for \(n,m \ge N_0\). Therefore, by Bolzano-Weierstraß Theorem 
it must converge 

\QED 

\subsection{Cauchy Sequences are Convergent}

Given a Cauchy sequence \(a_n\) then it converges 

\textbf{Proof:}

We know that \(a_n\) is bounded and by the Bolzano Weierstraß Therorem it has convergent subsequence. 
...
Prove that \(\sup \{\frac{n}{n + 1 \mid n \in \N}\} = 1\).  
...
Given a non-empty set \(A \subseteq \R\). The \(s \in Reals\) is only a supremum of \(A\) if 
...
A is considered \emph{monotone} if either for all \(n \in \N\) the following holds 
...
For each \(k \in \N\) exists some \(N(k) \in \N\) such that \(N(k) > N(k - 1)\)
and 
...
Other way of the proof, if the sequence \(\{x_{n_k}\}_{k = 1}^{\infty}\) converges towards \(x_0\) then there exists a \(\delta > 0\) and \(K \in \N\) 
such that 
...
Let \(a_n\) be a sequence and \(a_m\) be a peak if \(\forall a_i \in a_n, i > m, m, i \in \N\) the following holds \(a_i \le a_m\). 
...
\subsection{\texorpdfstring{Proof that \(-1^n\) is divergent}{}}
...
For all \(n \in \N\) with \(n \ge n_0 \in \N\). If  \(a_n \to 0, n \to \infty\) then  
\(b_n \to 0, n \to \infty\).
...
If \((a_n)\) is a sequence of complex numbers, where \(a_n = x_n + i y_n\) with \(x_n, y_n \in \R\), then the conjugate of \((a_n)\) is a new sequence \((\overline{a_n})\) where each term \(\overline{a_n}\) is the complex conjugate of \(a_n\):
