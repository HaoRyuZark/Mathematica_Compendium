...
\emph{Limits} are a foundational concept in calculus and analysis. They describe the behavior of 
...
Given a neighborhood \(U\) of \(x_0\), then \(\dot{U}:= U \setminus \{x_0\} \) is the \emph{pointed neighborhood} 
of \(x_0\). For \(\delta > 0\), we define 

\[
    \dot{U_{\delta}}(x_0) = (x_0 - \delta, x_0 + \delta) \setminus \{x_0\}
\]

Using this, we can then define the limit as follows.

Given \(f: D \to \R\) with \(x_0 \in D\) being an accumulation point, \(L\) is the limit if 
...
\section{Limit Calculation Rules}

\begin{itemize}
     
    \item \emph{Sum Rule:} \( \lim_{x \to a} (f(x) + g(x)) = \lim_{x \to a} f(x) + \lim_{x \to a} g(x)\).

    \item \emph{Difference Rule:} \( \lim_{x \to a} (f(x) - g(x)) = \lim_{x \to a} f(x) - \lim_{x \to a} g(x)\).

    \item \emph{Product Rule:} \( \lim_{x \to a} (f(x)g(x)) = \lim_{x \to a} f(x) \cdot \lim_{x \to a} g(x)\).

    \item \emph{Quotient Rule:} \(\lim_{x \to a} \frac{f(x)}{g(x)} = \frac{\lim f(x)}{\lim g(x)}\) if \(\lim g(x) \ne 0\).

    \item \emph{Power Rule:} \( \lim_{x \to a} {f(x)}^n = {(\lim_{x \to a} f(x))}^n\).

    \item \emph{Root Rule:} \( \lim_{x \to a} \sqrt[n]{f(x)} = \sqrt[n]{\lim_{x \to a} f(x)}\) (when defined).

    \item \emph{Logarithm Rule:} \( \lim_{x \to a} \ln(f(x)) = \ln(\lim_{x \to a} f(x))\) (when defined).

    \item \emph{e limit Rule:}   \(\lim_{x \to 0} (1 + f(x))^{\frac{1}{f(x)}} = e\).

\end{itemize}

\textbf{Proof of the Sum Rule:}

For any \(\varepsilon > 0, \text{ there exists a } \delta_1 > 0\) such that if \(0 < |x - a| < \delta_1\) then \(0 < |f(x) - L| < \varepsilon/2\). 
The same goes for \(g(x)\) with its respective limit and delta \(\delta_2\). Now choose \(\delta = \min\{\delta_1, \delta_2\}\) such that
...
Find the limit of \(\lim_{x \to 0} \frac{e^x - 1}{x}\).
...
The trend for \(x \to 0 \) tells us that \(z \to 0\) because \(\lim_{x \to 0} z = \lim_{x \to 0} e^x - 1 = 0\).

Our limit is now 

\[
    \lim_{z \to 0} \frac{z}{\ln(z + 1)} = \lim_{z \to 0} \frac{1}{\frac{1}{z}\ln(z + 1)} = \lim_{z \to 0} \frac{1}{ \ln\left((z + 1)^{\frac{1}{z}}\right)}
\]

The limit of a logarithm is the limit of the logarithm, and we also use the quotient rule.

\[
    \frac{1}{ \ln\left(\lim_{z \to 0}(z + 1)^{\frac{1}{z}}\right) = \ln(e)} = \frac{1}{\ln e} = 1
\]

\section{Limits at Infinity}

\[
    \lim_{n \to \infty} f(n) \iff \forall \varepsilon > 0, \exists N > 0 : \forall n > N: |a_n - L| < \varepsilon 
\]
...
\[
    \lim_{n \to \infty} \frac{1}{n} = 0, \quad \lim_{n \to \infty} e^n = \infty, \quad \lim_{n \to \infty} \frac{1}{n^2} = 0
\]

\textbf{Example:}

Prove that \(\lim_{n \to \infty} \frac{2n + 1}{3n + 7} = \frac{2}{3}\).

We start with the definition. Let \(\varepsilon > 0\) be given. We need to find \(N > 0\) such that for all \(n > N\): 

\begin{align*}
    \left| \frac{2n + 1}{3n + 7} - \frac{2}{3} \right| &< \varepsilon \\ 
    \left| \frac{(6n + 3) - (6n + 14)}{9n + 21} \right| &< \varepsilon \\ 
    \left| \frac{-11}{9n + 21} \right| \le  \frac{|-11|}{|9n| + |21|} =  \frac{|-11|}{9n + 21} < \frac{11}{9n} &< \varepsilon
\end{align*}
...
\emph{Indeterminate forms} arise in limits when the expression does not directly imply a limit value:

\[
    \frac{0}{0}, \quad \frac{\infty}{\infty}, \quad 0 \cdot \infty, \quad \infty - \infty, \quad 1^\infty, \quad \infty^0, \quad 0^0
\]

\section{Limit of the n-th Root of n}

We will show that \(\lim_{n \to \infty} \sqrt[n]{n} = 1\).

\textbf{Proof:}

Let \(n = (1 + (\sqrt{n} -1))^n\). This is a valid substitution. 

Now let us use the binomial theorem \((1 + (\sqrt{n} -1))^n = (1 + b_n)^n\).  
...
\section{Limit of a Composite Function}

If \( \lim_{x \to a} f(x) = L\) and \( \lim_{x \to L} g(x) = g(L)\) (i.e., \(g\) is continuous at \(L\)), then:
...
If and only if, for a sequence in the domain of \(f\), the following is true
...
where \(|f(a_n) - L| \ge \varepsilon\). Therefore, by letting \(\delta_n = \frac{1}{n}\), \(\exists a_n \in \mathcal{D}\) satisfying 
\(0 < |a_n - x_0| < \frac{1}{n}\) for which \(|f(a_n)- L| \ge \varepsilon\). 
...
\section{Limit of a Recursive Sequence}
...
\section{Squeeze Theorem (Sandwich Theorem)}
...
\section{The Number \texorpdfstring{\(e\)}{e} as a Limit}
...
\section{Limits of \texorpdfstring{\(\arctan\)}{arctan}}
...
Every bounded, convergent sequence in \(\C\) has a convergent subsequence.
More precisely, the sequence \(a\) has a biggest and smallest accumulation point, \(h^*\) and \(h_*\).

\[
    \forall \varepsilon > 0 : h_* - \varepsilon < a_n < h^{*} + \varepsilon \text{ for almost all } 
    b \in \N
\]
...
Let \( f: D \subset \R \to \R \), and suppose \(a\) is a limit point of \( D \). Then:
...
\section{Every Convergent Sequence Is a Cauchy Sequence}
...
\[
    \forall \varepsilon > 0, \ \exists N \in \N \text{ such that } \forall n, m \ge N, \quad |a_n - a_m| < \varepsilon
\]
...
\[
    x_n := \sup \{a_k \mid k \ge n\} \quad y_n := \inf \{a_k \mid k \ge n\}
\]
