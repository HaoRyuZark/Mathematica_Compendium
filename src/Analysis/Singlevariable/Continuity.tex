...
Let \( f : D \to \R \) be a function and \( x_0 \in D \).
...
If \(f: [a, b] \to \R\) is continuous on the closed interval \([a, b]\), then exist \(x_m, x_M in [a,b]\) such that 
...
If for set \(A \subseteq \R\) with \(f : A \to \R\) holds that 
...
Given a continuous function \(f:[a,b] \to \R\) with \(f([a,b]) \subseteq [a,b]\), then there exists at least one fixed 
