...
\section{Definition of Continuity at a Point}
...
In other words, a real-valued function \(f\) is continuous if it is continuous at all points in its 
domain.

\subsection{Approach to Solve Problems}
...
\begin{enumerate}
    ...
    \item Substitute the concrete function for \(f\) in the definition.

    \item Modify the expression with the concrete \(f\), \(|f(x) - f(y)|\), in such a way that you get 
          \(|x - y|\) inside the expression.
    
    \item Choose a \(\delta\) if not given. On one hand, it is often \(\delta = |x - y|\), and then 
          substitute it into the expression. On the other hand, if \(\delta\) is given, make an approximation 
          which is greater than the current expression and then simplify. In both cases, after simplifying, 
          you should get an \(\varepsilon\) if the function is continuous. 
...
\begin{align*}
    |f(x) - f(0)| &= |f(x) - 0| = |x^2 \sin \left(\frac{1}{x}\right)| \\
    &\le |x^2| \cdot \left|\sin \left(\frac{1}{x}\right)\right| \\
    &\le |x^2| \cdot 1 = x^2 < \varepsilon \\ 
    &\Rightarrow x < \sqrt{\varepsilon}
\end{align*}
...
\section{Characterization Using Sequences}

Let \(f : (a,b) \to \mathbb{R}\) be a function and \(x_0 \in (a,b)\).

The function \(f\) is continuous at \(x_0\) if and only if for every \emph{sequence} 

\[
    \{x_n\}_{n\in \N}
\] 

with \(x_n \in (a,b)\) for all \(n \in \N\) and \(\lim_{n\to\infty} x_n = x_0\), it holds that 
...
Let \( f : D \to \R \) be a function and \( x_0 \in D \).The function \( f \) is called \emph{left-continuous} at \( x_0 \) if
\( f \) is continuous on \( D \cap (-\infty, x_0] \).

Analogously, \( f \) is \emph{right-continuous} at \( x_0 \) if\( f \) is continuous on \( D \cap [x_0, \infty) \).

\section{Rules of Continuity}
...
A function on an interval \([a,b]\) which is continuous is \emph{bounded}.

\textbf{Proof:}

If \(f\) is not bounded, then \(\forall n \in \N, \exists x_n \in [a,b]\) such that
\[
    |f(x_n)| > n
\]
...
But by construction \(|f(x_{n_k})| > n_k \to \infty\) as \(k \to \infty\), which is a contradiction to 
\(x^* \in \mathcal{D}(f)\).
...
If \(f: [a, b] \to \R\) is continuous on the closed interval \([a, b]\), then there exist \(x_m, x_M \in [a,b]\) such that 
...
Let \(m := \inf_{[a,b]} f, \quad M := \sup_{[a,b]} f\). We know by the previous theorem that \(f\) is bounded, so \(m, M \in \R\).
...
If for a set \(A \subseteq \R\) with \(f : A \to \R\) it holds that 
...
\section{Middle Value Theorem}

If \(f: [a, b] \to \R\) is continuous and \(f(a) \ne f(b)\), then for every \(y_0\) between \(f(a)\) and \(f(b)\) 
there exists \(x_0 \in (a, b)\) such that \(f(x_0) = y_0\).

\section{Continuity of the Inverse by a Decreasing Function}

Given a monotonically decreasing continuous function \(f\) on \([a, b]\) with \(f(a) = c\) and \(f(b) = d\), the inverse function 
\(f^{-1}: [d, c] \to [a, b]\) is also continuous and monotonic.

\textbf{Proof:}

\subsection{Continuity of the Root and Logarithm Function}
...
A function \(f: D \subset \R \to \R\) is \emph{Lipschitz continuous} if:
...
\section{Fixed Point on Interval}

Given a continuous function \(f:[a,b] \to \R\) with \(f([a,b]) \subseteq [a,b]\), there exists at least one fixed 
...
\subsection{A Priori and A Posteriori Approximations}
...
\subsection{Steps for a Fixed Point Exercise}
...
In this case, we will assume that it is monotonic. 
...
A function \(f: \mathcal{D} \subset \R \to \R\) is \emph{uniformly continuous} if:
...
Since \(\frac{1}{x} > 1\) for \(x < 1\), we have a contradiction, and therefore \(f\) is not uniformly continuous on \((0,1)\).

\section{Boundedness by Uniform Continuity}

If a function \(f: (a, b) \to \R\) is uniformly continuous on the open interval \((a, b)\), then \(f\) is bounded.

\section{Cauchy Sequence by Uniform Continuity}

Given \(\mathcal{D} \subseteq \R\) and a uniformly continuous function \(f: \mathcal{D} \to \R\),
for every \emph{Cauchy sequence} \(\{x_n\} \subseteq \mathcal{D}\), the sequence \(\{f(x_n)\}\) is also a Cauchy sequence.
