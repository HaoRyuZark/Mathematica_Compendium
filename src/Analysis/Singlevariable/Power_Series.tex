...
Many functions can be represented as \emph{power series}, which is useful for various applications in calculus 
and analysis. This chapter discusses some common techniques for transforming functions into power series.

\section{Common Cases}

Here are some \emph{common cases} where functions can be easily transformed into power series:
...
First, we perform \emph{partial fraction} decomposition. We can rewrite the function as:

\[
    \frac{x + 1}{3 - x} = \frac{-(3 - x) + 4}{3 - x} = -1 + \frac{4}{3 - x}.
\]

\subsection{Manipulating the Expression}
Now, we \emph{manipulate} the fraction to fit the form \(\frac{1}{1 - u}\):
...
Therefore, the \emph{power series representation} of the function is
