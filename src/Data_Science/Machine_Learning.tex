...
predictions based on gathered data and mathematical models. It uses heavily \emph{data science} which is another
area of computer science with the goal of analyzing data, identify patterns and perform statistical analysis.
...
If a model is trained only used the same data again, and again it will become
...
\begin{itemize}
	...
	\item \emph{Econding Non-numerical Values}: Categorical data and other non-numerical features need to be encoded or discard, depending on the context.

	\item \emph{Dropping/Replcing NaN-Values}: Either we discard columns or rows which contain incomplete data or we fill the gaps using some placeholder values, which 
	should have an neutral efect during the training process, like 0.  

\end{itemize}

\subsection{Exporative Data Analysis}
...
the goal of establishing hypothesis about the data for further analysis. This step is to be done extensively before any real 
training of the data.

\subsection{Feature Engineering}

\emph{Feature engineering} is the process of selecting the right features/predictors using domain-knowledge. This
is procedure is also accompanied with the selection of the right mathematical model for the desired goal.

The main difference from the explorative data analysis is that the goal is to create data-drive models, i.e. mathematical models
...
\emph{Hyper-parameter tunning} is the process of improving the hyper-parameters via validations methods. In such methods not only is the model
...
Number correclty negative classified observation in relation to the correctly and not correctly classified  negative observations.
Ideally high.
...
\[
	\mathrm{F1} = 2 \frac{\mathrm{Precission} \ \mathrm{Recall}}{\mathrm{Precission} + \mathrm{Recall}}
\]
...
\(y_i = \mathrm{Recall}_\theta, \, x_i = \mathrm{Specificity}_\theta\), where \(\theta\) is threshold for 
a classification problem.

When we read a value like: \((0.8, 0.2)\) we can say that 80 percent were correctly classified and 20 percent were missclassified.

The higher the curve is the better is our classification model. This can be compared using the area under the curve ROC of different 
curves.
...
\begin{itemize}
	...
	\item \emph{Validation Data} \(\mathcal{D}_V\): Used for testing our model against other models in situations like hyper-parameter tunning.
...
\begin{itemize}
	...
	which is the number of missclassified samples.
...
\emph{\(V\)-Cross Validation} is a technique to improve a models accuracy by presenting training the model on different splits on the data.
...
\begin{itemize}
	...
	\item \(e_n:= E_{\text{val}} (\overline{g_n}) = e(\overline{g_n}(x_n), y_n)\), which, again, is the number of missclassified samples.

	\item \emph{Cross Validation Error} \(E_{\text{V-fold CV}} = \frac{1}{V} \sum_{n = 1}^{V} E_v\)

\end{itemize}

Cross validation is mainly used for \emph{model selection}, which is process of comparing different machine learning algorithms, and \emph{hyper-parameter-tunning}, not to directly deliver a model. 

\subsection{Hold-Out Validation}

\emph{Hold-Out validation} works by just splitting the data set into traing und validation data. We only get one model.
...
The simplest model which approximates the data is also the more plausible. Hence, we should aim for the simplest model (least parameters possible) which 
approximates the data good enough.
...
have all possible samples which will exists. But we can estimate this error using the split of the training and validation data by 
...
\emph{Generalization:} \(E_{\text{out}} \approx E_{\text{in}}\). Note t

\subsection{Bias and Variance}

Fitting the training data well, but making poor predictions, is called the \emph{bias-variance} trade-off.

The \emph{bias} measures how well our model differs from our training data (ground truth) while \emph{variance} tells us how well 
our model can make feature predictions. A high bias leads to \textbf{underfitting} while a high variance to \textbf{overfitting}.
...
Underfitting can be handled by: using more data for the training, reduce regularization, feature selection, etc.
...
where \(\lambda\) is the parameter for how much we should regularize, \(w^T w\) is the punish-term based on the weight of the vector and
\(N\) is the number of data points.

\section{Regularization}

\emph{Regularization} is the process of making the algorithm less sensible to out-of-sample data.
It helps the learning algorithm to minimize the output-of-sample error. This is accomplished by different methods like adding error terms of a 
...
It is important to note that both this and Ridge regularization is \textbf{very sensible} to the scaling of our features.
...
Yet, this is computationally very expensive and does not gives us \(b\). An alternative way is by minimizing the \emph{mean squared error} function given by
...
We choose this because for each of the samples we a discrete \emph{geometric distribution}.
...
\emph{Nearest neighbors models NNM} used te labels of the neighbors in the proximity to perform classification or regression.
...
The \emph{KNN regression} is determines the label of the feature based on the \textbf{mean} of the
...
Hence, for the maximization of the margin we need to minimize \(\metric{w}\) with the constraings that 
...
By putting our new terms in our orignal equation we get that 
...
Formally, we wnt to minimize \(\frac{1}{2} w^T w + \frac{\gamma}{2} \sum_n^N e_n^2\) under \(N\) constraints.
...
while minimizing the loss of information.
...
The non-Euclidean MDS variants do not have a closed form solution, they are iterative methods; hence, we will present the classical MDS with the Euclidean distance.
...
The goal of the \emph{linear discriminant analysis} is to find directions \(u_1, u_2, \ldots, u_k\) such that they can be best separate 
...
For quantifying the separation of the points \textbf{projected onto \(u_i\)} we define the following
...
The bigger the mutual information the similar.
...
the mean distance to \(x_i\) to all other points in the cluster.
...
the minimal mean distance between \(x_i\) to all other points in the closest cluster.
