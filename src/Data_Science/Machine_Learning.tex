...
\emph{Machine Learning} is an area of computer science which focuses on making accurate
predictions based on gathered data and mathematical models. It heavily uses \emph{data science}, which is another
area of computer science with the goal of analyzing data, identifying patterns and performing statistical analysis.

\section{Core Vocabulary}

Our main terminologies and notation are:

\begin{itemize}

	\item Historical Data which refers to the data on which we are going to base our model.

	\item Feature/Predictor vector \(X\): This is a vector whose components represent the different numeric values of the characteristics
	of some object.

	\item Function \(f\): This is called the \emph{target function} \(f: \mathcal{X} \to \mathcal{Y}\), which makes a perfect approximation.

	\item Label \(y\): This is the image of some \(x \in \mathcal{X}\).
...
continuous values, i.e., identifying trends and patterns.
...
The data set \(\mathcal{D}\) will be split into \emph{training} and \emph{test} data to minimize our
margin of error. 

If a model is trained using only the same data again and again, it will become
...
\subsection{Data Preparation Techniques}

\begin{itemize}
	\item \emph{Train-Test-Split}: Splitting the data set into the data used for training and validation.

	\item \emph{Encoding Non-numerical Values}: Categorical data and other non-numerical features need to be encoded or discarded, depending on the context.

	\item \emph{Dropping/Replacing NaN-Values}: Either we discard columns or rows which contain incomplete data or we fill the gaps using some placeholder values, which 
	should have a neutral effect during the training process, like 0.  

\end{itemize}

\subsection{Explorative Data Analysis}

\emph{Explorative data analysis} is the process of analyzing, visualizing and exploring the data using software, statistical methods, intuition, etc., with
the goal of establishing hypotheses about the data for further analysis. This step is to be done extensively before any real 
training of the data.

\subsection{Feature Engineering}

\emph{Feature engineering} is the process of selecting the right features/predictors using domain-knowledge. This
procedure is also accompanied by the selection of the right mathematical model for the desired goal.

The main difference from the explorative data analysis is that the goal is to create data-driven models, i.e., mathematical models
from our data to make predictions instead of just exploring and analyzing the data.
...
A \emph{hyper-parameter} is a parameter used by a model/algorithm which cannot be approximated directly during the training process.
For example, the \(k\) in K-Means is given at the start.

\subsection{Hyper-Parameter Tuning}

\emph{Hyper-parameter tuning} is the process of improving the hyper-parameters via validation methods. In such methods not only is the model
tested against other models with the same hyper-parameters, but also against the same model with different hyper-parameters. We do this via 
cross-validation among other techniques.

\section{Accuracy Metrics for Classification}
...
For example, in a binary classification problem, the confusion matrix is a \(2\times2\) table that summarizes the counts of true positive \(TP\),
...
\begin{center}
	...
	\hline
			 & Positive 		& Negative 		 \\ 
	\hline
	Positive & \(\mathrm{TP}\) & \(\mathrm{FN}\) \\ 
	\hline
	Negative & \(\mathrm{FP}\) & \(\mathrm{TN}\) \\ 
	\hline
\end{tabular}
...
The \emph{accuracy} is then given by

\[
	\mathrm{Acc} = \frac{TP + TN}{TP + TN + FP + FN}.
\]

Portion of correctly predicted observations to the total observations.
Ideally high.

\subsection{Precision}

The \emph{precision} is then given by

\[
	\mathrm{Precision} = \frac{TP}{TP + FP}.
\]

Ratio of the correctly predicted positive observations to the total predicted positive observations.
Ideally high.

\subsection{Recall}

The \emph{recall} or true positive rate (TPR) is then given by

\[
	\mathrm{Recall} = \frac{TP}{TP + FN}.
\]

Ratio of real positives to the total of positives. Ideally high.

\subsection{Specificity}

The \emph{specificity} or true negative rate (TNR) is then given by

\[
	\mathrm{Specificity} = \frac{TN}{TN + FP} = \frac{TN}{N}.
\]

Number of correctly classified negative observations in relation to the correctly and incorrectly classified negative observations.
Ideally high.

\subsection{The F1 Metric}

The \emph{F1 metric} is a measure which combines two of the previous metrics into one: 
...
A \emph{ROC curve} is a graphical representation of a set of points \((y_i, x_i)\) with 
\(y_i = \mathrm{Recall}_\theta, \, x_i = \mathrm{Specificity}_\theta\), where \(\theta\) is the threshold for 
a classification problem.

When we read a value like \((0.8, 0.2)\), we can say that 80 percent were correctly classified and 20 percent were misclassified.

The higher the curve, the better our classification model. This can be compared using the area under the ROC curve of different 
curves.

\section{Accuracy Metrics for Regression}

\subsection{R-Squared}

\emph{R-squared} provides a measurement of how much variation in the response the model explains.
...
This metric is not available for all models. Linear regressions, among others, support this metric.

A problem of this metric is that it increases when more predictors are added, which means that even irrelevant coefficients are
taken into consideration.

\subsection{Adjusted R-Squared}

This is a penalized version of \(R^2\) to avoid taking irrelevant predictors.

\[
	\text{adjusted }R^2 = 1 - (1 - R^2) \frac{N - 1}{N - p},
\]

with \(N\) as the sample size and \(p\) as the number of coefficients in our linear model.

\subsection{Mean Squared Error}

This is the classical function for our linear regression given by
...
\emph{Validation} refers to the process of evaluating our models to approximate the out-of-sample error, compare them, and find
...
\begin{itemize}
	...
	\item \emph{Training Data} \(\mathcal{D}_T\): Data used for training the model.

	\item \emph{Validation Data} \(\mathcal{D}_V\): Used for testing our model against other models in situations like hyper-parameter tuning.
	We normally use a size of \(K\) datapoints where \(K = N/5\).

\end{itemize}

\subsubsection*{Quote From the Statistical Learn-Theory}

The more often you use a data-set for training a model, taking decisions, etc., the harder it is to approximate
the capacity of the data-set to approximate the out-of-sample error.

\subsection{Leave One Out Cross Validation}

\emph{Leave One Out Cross Validation} works by always leaving \textbf{one data point out}, instead of a fold.
This is the same as an \(N\)-fold cross validation.

The nomenclature:

\begin{itemize}

	\item \(e_n:= E_{\text{val}} (\overline{g_n}) = e(\overline{g_n}(x_n), y_n)\), represents the point-wise error,
	which is the number of misclassified samples.

	\item \emph{Cross Validation Error} \(E_{\text{CV}} = \frac{1}{N} \sum_{n = 1}^{N} e_n\)

\end{itemize}

\subsection{\texorpdfstring{\(V\)-Cross Validation}{V-Cross Validation}}

\emph{\(V\)-Cross Validation} is a technique to improve a model's accuracy by training the model on different splits of the data.

The procedure consists of splitting the data-set into \(V\) different sub-sets of equal size called \emph{folds}. Then, we train our model \(V\)-times, each time using \(V-1\) disjoint sub-sets for
training and the remaining one for testing. The final out-of-sample error is given by the average of all the out-of-sample errors obtained from each iteration.
We will have in total \(V\) models.
...
\begin{itemize}
	...
	\item \(e_n:= E_{\text{val}} (\overline{g_n}) = e(\overline{g_n}(x_n), y_n)\), which, again, is the number of misclassified samples.

	\item \emph{Cross Validation Error} \(E_{\text{V-fold CV}} = \frac{1}{V} \sum_{n = 1}^{V} E_v\)

\end{itemize}

Cross validation is mainly used for \emph{model selection}, which is the process of comparing different machine learning algorithms, and \emph{hyper-parameter-tuning}, not to directly deliver a model. 

\subsection{Hold-Out Validation}

\emph{Hold-Out validation} works by just splitting the data set into training and validation data. We only get one model.
...
The simplest model which approximates the data is also the most plausible. Hence, we should aim for the simplest model (least parameters possible) which 
approximates the data well enough.

\subsection*{Corrupted Samples}

If the samples are corrupted, biased or contain errors, then the model will also not be neutral
and will give false approximations.

\subsection*{Data Snooping}

If a data-set affects the learning process, then its capacity to approximate the out-of-sample error
was reduced. Therefore, we should not mix test and training data.
...
The automatic variants do deliver accurate models on paper, but they may not reflect the reality of the function we are trying to
approximate, or they do not take into account certain relationships in the data. For these reasons, regularization methods are mostly preferred, but feature
selection is still useful if done correctly.

\subsection{Best Subset Selection}

\emph{Best subset selection} consists of testing all \(2^p\) possibilities of combining \(p\) features and then perform cross validation
...
Removing variables one at a time, choosing the one which contributes the least to the error function.
...
\[
	E_{\text{in}} (h) = \frac{1}{N} \sum_{i = 1}^{N} W(h(x_i) \neq f(x_i))
\]

where

\[
	W(\mathrm{Expr}) = \begin{cases}
...
have all possible samples which will exist. But we can estimate this error using the split of the training and validation data by 
...
give us the exact function because of the lack of data. We want to minimize the in-sample error
but also make our approximation as general as possible.
...
The \emph{bias} measures how much our model differs from our training data (ground truth), while \emph{variance} tells us how well 
our model can make future predictions. A high bias leads to \textbf{underfitting} while a high variance to \textbf{overfitting}.
...
lines \(h(x) = b\) and \(\mathcal{H}_1\) the set of all lines \(h(x) = ax + b\). The training-sets are two data-points of the interval
\([-1,+1]\).
...
in which hypotheses \(h\) continuously reduce the in-sample error, but leading to a bigger out-of-sample error.
...
In summary, noise in the data (stochastic or deterministic) has a negative effect on the learning process and can lead
...
Underfitting can be handled by: using more data for the training, reducing regularization, feature selection, etc.
...
where \(\lambda\) is the parameter for how much we should regularize, \(w^T w\) is the penalty term based on the weight of the vector and
\(N\) is the number of data points.

\section{Regularization}

\emph{Regularization} is the process of making the algorithm less sensitive to out-of-sample data.
It helps the learning algorithm to minimize the out-of-sample error. This is accomplished by different methods like adding error terms of a 
\emph{regularization vector} 

\[
	\beta = [\beta_0, \beta_1, \dots]
\]

with one or multiple parameters \(\lambda\).
...
instead of summing the squares we add the absolute value

\[
	\mathrm{LASSO}: h(x_i)  + \lambda \sum_{i = 1}^{p} | \beta_i |.
\]

It is important to note that both this and Ridge regularization are \textbf{very sensitive} to the scaling of our features.

Note that adding the absolute values is equivalent to defining a multidimensional \textbf{polygon}.

% Picture

\subsection{Ridge Regression L2}

This is a regularization technique used in linear models to prevent overfitting by discouraging the model from assigning
excessively large values to its parameters. 

To define the region in which we want our parameters to be we use a \textbf{hyper-sphere} as region by setting a sum of squares as a constraint.
...
We use linear regression for this derivation: 
...
\emph{The perceptron} is our first model in machine learning. It consists of having a separate vector \(w \in \R^d\) which consists of weights which are going to be
...
\section{The Perceptron Learning Algorithm (PLA)}

The \emph{pereptron learning algorithm (PLA)} modifies \(w\) until the data are linearly separated, as long it is possible.
...
Yet, this is computationally very expensive and does not gives us \(b\). An alternative way is by minimizing the \emph{mean squared error} 
function given by

\[
	\mathrm{MSE} = J(w,b) = \frac{1}{N} \sum_{i = 1}^{N} (y_i - (w^Tx_i + b))^2
\]
...
\[
	...
	=
	\begin{bmatrix}
		\frac{1}{N} \sum -2x_i (y_i -(w^Tx_i + b)) \\
		\frac{1}{N} \sum -2 (y_i -(w^Tx_i + b))
	\end{bmatrix}
	=
	\begin{bmatrix}
...
The two algorithms covered until now have the condition that both of them are linear, but
what if our data cannot be linearly separated? We can modify the input variable \(x_i\)
...
\emph{Logistic Regression} is a statistical model which maps a continuous variable to a discrete number of classes using the \emph{sigmoid} function 
and thresholds for the different classes. 
...
squish it between 0 and 1,  hence, we get probabilities.
...
We choose this because for each of the samples we have a discrete \emph{geometric distribution}.
...
Using the cross entropy we can define our error for logistic regression. By taking the gradient and applying
gradient descent.
...
\[
	\nabla_b \log L(w,b) = \frac{1}{N} \sum_{i=1}^{N} (h_\theta(x_i) - y_i).
\]
...
\emph{Decision trees} are used mainly for \emph{regression trees} or \emph{classification trees} which
are used to partition the data into subsets based on a purity measurement.
...
\emph{Information gain} is a measure commonly used in decision tree algorithms such as ID3, C4.5, and CART to determine which feature should be used to split the dataset
...
\[
    \mathrm{IG} = \mathrm{H}(P) - \sum w_i \mathrm{H}(C_i).
\]

where \(H(X)\) is the entropy and \(w_i = \frac{|C_i|}{|P|} \) the relative cardinality of the child with respect to the parent.

\subsection{Regression Trees}

A \emph{regression tree} divides the feature space into disjoint sub-spaces in such a way that it brings
the value of all labels \(y_i\) for \(x_i \in \mathcal{D}\) as close as possible to the true label \(\overline{y_i} \approx y_i\).
...
The process is similar to the one used for classification trees, but we use \emph{variance reduction} for our measurements.

\subsubsection{Variance Reduction}

The \emph{variance reduction} is given by 

\[
    \mathrm{VR} = \var(P) - \sum w_i \var(C_i),
\]

where \(w_i = \frac{|P|}{|C_i|}\).

\subsection{Recursive Binary Splitting}

We previously stated an optimization problem, which sadly cannot be solved efficiently; instead,
...
\emph{Nearest neighbors models NNM} use the labels of the neighbors in the proximity to perform classification or regression.

\subsection{Quantifying the Differences Between Features}

Examples of metrics, distance measurements:

\begin{itemize}

    \item \emph{Euclidean Distance}: \( d(x ,y) = \metric{x - y}\).

    \item \emph{Cosine Similarity}: \( \mathrm{sim}(x, y) = \frac{x \cdot y}{\metric{x} \metric{y}} \).

    \item \emph{Jaccard Similarity}: \( \mathrm{sim}(A, B) = \frac{|A \cap B|}{|A \cup B|} \in [0, 1] \).
...
Feature vectors in the training set are noted with their distance to \(x\). \(x_{[1]}\) symbolizes the nearest neighbor of
...
The \emph{Nearest Neighbor Model} gives us our final hypothesis \(g(x) = y_{[1]}(x)\).
...
\[
	h(x) = \mathrm{sign} \left(\sum_{i = 1}^{k} y_{[i]}(x)\right).
\]
...
The \emph{KNN regression} determines the label of the feature based on the \textbf{mean} of the
label of the neighbors and a series of thresholds.

\[
	h(x) = \frac{1}{k} \sum_{i = 1}^{k} y_{[i]}(x).
\]
...
\begin{enumerate}
	...
	\item Compute its distance to all other data points \(d(z, v_i)\).
	...
	\item Depending on regression or classification, apply one of the following formulas

	      \[
		      \mathrm{Regression:}\, h(x) = \mathrm{sign} \left(\sum_{i = 1}^{k} y_{[i]}(x)\right) \quad \mathrm{Classification:}\, h(x) = \frac{1}{k} \sum_{i = 1}^{k} y_{[i]}(x)
	      \]

	      using the indexes of the \(k\) nearest neighbors for getting the corresponding labels.
...
\begin{itemize}
	...
	\item \emph{RBF:} \(k(x,y) = \exp( -\gamma \metric{x -  y}^2)\)
...
\emph{Support vector machines} are a family of algorithms which use the perceptron as a basis for classifying points into classes using a hyperplane
...
The size of the margin is given by the normal vector \(w\) in the following way: Imagine, that we want to take a step 
into the direction of \(w\) from a point \(x\) on our decision boundary (\(w^T x + b  = 0\)), such that we reach the portion of the other class.
We also make the assumption that for a point of the first class \(w^T x_{+}  + b = 1\), \(w^T x_{-} + b = -1\).

\begin{align*}	
	w^T(x + k \frac{w}{\metric{w}}) + b &= 1 \\
	w^Tx + w^T k \frac{w}{\metric{w}} + b &= 1 \quad \text{note that}\, (w^Tx + b = 0) \\ 
	k \frac{w^T w}{\metric{w}} &= 1 \\ 
	k  &= \frac{1}{\metric{w}} 
\end{align*}

Therefore, the length taking into consideration both directions is \(\frac{2}{\metric{w}}\).

Hence, for the maximization of the margin we need to minimize \(\metric{w}\) with the constraints that 

\begin{align*}
	y_i = 1 \implies w^T x + b &\ge 1 \iff x \in C_1 \\ 
	y_i = -1 \implies w^T x + b &\le -1 \iff x \in C_2.
\end{align*}
...
This comes from the fact that \(1 (1) \ge 1\), \(-1 (-1) \ge 1\) given our rules. Thus, we combine both into 
one expression.

\subsection{Hard Margin SVM}

A \emph{hard margin SVM} is a perceptron with maximal margin and no misclassifications allowed.

Formally, we want to solve the following optimization problem:

\emph{Minimize:} \(\frac{1}{2}\metric{w}\) under \(N\) \emph{constraints} \(y_i (w^T x_i + b) \ge 1\).

\emph{Maximize:} \(y_n (w^T x_n + b) \ge 1, (n = 1, \dots, N)\).

First, we use the technique of Lagrange multipliers giving us

\[
	\mathcal{L}(w, \alpha_1, \dots, \alpha_n, b) = \frac{1}{2} \inner{w}{w} - \sum \alpha_i [y_i (\inner{w}{x} + b) - 1].
\]

Giving us 

\[
	\frac{\partial \mathcal{L}}{\partial w} = w - \sum \alpha_i y_i x_i = 0 \iff w = \sum \alpha_i y_i x_i
\]
...
By putting our new terms in our original equation we get that 

\[
	\mathcal{L}(w, \alpha_1, \dots, \alpha_n, b) = \frac{1}{2} \left(\sum \alpha_i y_i x_i\right) \left(\sum \alpha_i y_i x_i\right) - 
	\sum \alpha_i \left[y_i \left(\inner{\sum \alpha_i y_i x_i}{x} + b\right) - 1\right].
\]

Giving us 

\[
	- \sum \alpha_i - \frac{1}{2} \sum_i \sum_j \alpha_i \alpha_j y_i y_j \inner{x_i}{x_j}.
\]

This is the so-called \emph{dual optimization problem}.
...
where \(C\) is our regularization parameter.
...
We will allow non-linear feature Transformations \(x \to \Phi(x)\) such that
...
Let us also define \(K:= \Phi(x_n)^T \Phi(x_i)\).

\subsection{Least Squares Support Vector Machine}

This is a simplified version of the soft margin SVM.

Formally, we want to minimize \(\frac{1}{2} w^T w + \frac{\gamma}{2} \sum_n^N e_n^2\) under \(N\) constraints.
...
Given \(N\) feature vectors with \(D\) features, the \emph{average vector} is given
by
...
This vector is used to center the data by subtracting it from each feature vector. This vector has as its components the mean of the corresponding 
feature across all feature vectors.

\section{Z Scoring}

Given \(\overline{X}\) as the mean and \(\sigma\) as the standard deviation of \(X\). The standardized feature
...
The \emph{curse of dimensionality} is a concept that describes the problems which arise when dealing with high-dimensional data. A high
...
To reduce dimensionality we have two categories of methods: \emph{linear} and \emph{non-linear}. The linear methods use the assumption that the data lies on a linear subspace of the original feature space which
means that they use, mostly, the Euclidean distance as a measure of similarity. Non-linear methods take into consideration the topology of the data and use non-linear transformations to reduce the dimensionality.

In the next sections we will cover some algorithms for this purpose.
...
\[
	x' = \mathrm{Proj}_{u}(x) = \frac{\inner{x}{u}}{\inner{u}{u}} u = \inner{x}{u} u \quad \text{with } \metric{u} = 1.
\]

We can use the term \(\inner{x}{u}\) as the measure of \textbf{information preserved between the two vectors \(x\) and \(x'\)}. This is
maximal when they are \textbf{parallel} and minimal when they are \textbf{orthogonal}.

For that, we use the variance of the projections with our unit vector \(u\):

\begin{align*}
	\frac{1}{N} \sum_{i = 1}^{N} (u^T x_j -  u^T \overline{x})^2 & = \frac{1}{N} \sum_{i = 1}^{N} [u^T (x_j - \overline{x})]^2                           \\
	                                                                 & = \frac{1}{N} \sum_{i = 1}^{N} u^T (x_j - \overline{x}) u^T(x_j - \overline{x})     \\
	                                                                 & = \frac{1}{N} \sum_{i = 1}^{N} u^T (x_j - \overline{x}) (u^T(x_j - \overline{x}))^T \\
	                                                                 & = \frac{1}{N} \sum_{i = 1}^{N} u^T (x_j - \overline{x}) (x_j - \overline{x})^Tu_i     \\
	                                                                 & =  u^T S u.
\end{align*}

Hence, we want to maximize the sum of the inner products between the original data and the projected data onto the directions \(u\), while minimizing the loss of information.
...
note that this is maximal when \(\lambda_i\) is maximal.
...
\subsection{Projection of the Data After the Analysis}
...
When we multiply our matrices \(X\) and \(W\) we get
...
which essentially describes our vector as a linear combination:
...
this problem we make use of one of the following properties of our eigenvalue problem.
...
This means that our eigenvalues and eigenvectors are the same if, instead of the \emph{feature covariance matrix}, we use the \emph{sample covariance matrix} \(XX^T\).
...
\subsection{Reformulation of the PCA With Non-Linear Features}
...
\subsection{Eigenvector as Linear Combination of the Transformed Features}
...
\subsection{Find the Eigenvectors}
...
\subsection{Normalization of the Eigenvectors}
...
\subsection{Projection Onto the KPCA Axis}
...
\subsection{Centering of the Features}
...
\begin{enumerate}
	...
	\item Solve the eigenvalue problem \(\hat{K} \vec{\alpha}_j = \hat{\lambda}_j \vec{\alpha}_j\) to find the eigenvalues \(\hat{\lambda}_j\) 
	and eigenvectors \(\vec{\alpha}_j\).
...
\emph{Multidimensional Scaling (MDS)} is a family of procedures for the visualization and dimension reduction of features.
The non-Euclidean MDS variants do not have a closed form solution, they are iterative methods; hence, we will present the classical 
MDS with the Euclidean distance.
...
In this classical approach \(d_{i,j}\) represents the distances in the low-dimensional configuration, \(\delta_{i,j}\) the observed distances or dissimilarities and
...
Using the fact that we can write our inner products as
...
Finally, reconstruct \(X\) using the eigendecomposition with \(V\) as the matrix of eigenvectors and \(\Lambda\) the eigenvalue matrix:
...
The idea is similar to PCA, but instead of using correlation between samples, we use the distance/metric to group samples. Choosing the right
...
\subsection{Proportion of Distance Matrix Explained (PDME)}

The \emph{PDME} is given by: 

\[
	\mathrm{PDME} = \frac{\lambda_j}{\sum_{i = 1}^{N} | \lambda_i |}.
\]

\subsection{Euclidean Constraint}
...
This is mostly not a problem in practice, since the negative eigenvalues and their associated eigenvectors are discarded during the algorithm, but
this can limit the possibilities of projecting our data, meaning more loss of information.

\section{Linear Discriminant Analysis}

The goal of the \emph{linear discriminant analysis} is to find directions \(u_1, u_2, \ldots, u_k\) such that they best separate 
our classes. It is also a dimension-reduction technique, in which we want to maximize the separability of the classes.
...
\subsection{Mean Vector of a Class}
...
It is important to note that this is a supervised learning method.

\subsection{Within-Class Scatter}

For quantifying the separation of the points \textbf{projected onto \(u_i\)} we define
...
For the \(n-\)classes we have

\[
	S_W = \sum_{k = 1}^{n} S_{wk} = \sum_{k = 1}^{n} \sum_{n \in \mathcal{C}_k} (x_n - m^{(k)})(x_n - m^{(k)})^T.
\]

\subsection{Between-Classes Scatter}

The quantification of the separation between \textbf{the middle points}, called the
\emph{between-classes-scatter} is given by

\begin{align*}
	(\hat{m}^{(1)} - \hat{m}^{(2)})^2 & \to (u_1^T m^{(1)} - u_1^T m^{(2)})^2                  \\
...
\[
	S_b = \sum_{k = 1}^{n} \sum_{n \in \mathcal{C}_k} (m^{(n)} - m^{(k)})(m^{(n)} - m^{(k)})^T.
\]

\subsection{Fisher Discriminant}

The \emph{Fisher Discriminant} is the ratio of the \textbf{between-class scatter} and the \textbf{within-class scatter}.
In this case we will do the derivation by considering only two classes.
...
\subsection{Linear Discriminant Algorithm}

\begin{enumerate}

	\item Compute the within-class scatter matrix \(S_W\) and the between-class scatter matrix \(S_b\).
	\[
		S_W =  \sum_{k = 1}^{n} \sum_{n \in \mathcal{C}_k} (x_n - m^{(k)})(x_n - m^{(k)})^T \quad S_b = \sum_{k = 1}^{n} \sum_{n \in \mathcal{C}_k} (m^{(n)} - m^{(k)})(m^{(n)} - m^{(k)})^T.
	\]
...
\begin{itemize}
	...
	\item  \(\prob(x_i \mid y)\) is the class conditional probability which is modeled with a normal distribution.
...
\emph{Clustering} is a data analysis technique aimed at partitioning the data into mostly disjoint sets 
such that the elements of each set exhibit a great similarity based on the measurement chosen.

It is important to note that clustering algorithms always find clusters, no matter the data given. Therefore, it is the responsibility of 
the data scientist to analyze the results and determine if the clusters are meaningful or not.
...
\[
	J = \frac{1}{|C_i|} \sum_{i = 1}^{k} \sum_{x_j \in C_i} \metric{ x_j - \mu_i}^2.
\]
...
but due to it being a rather complex optimization problem, K-Means is computed in an iterative manner.

\subsection{K-Means Algorithm}

\begin{enumerate}

	\item Initialize centroids randomly by choosing a certain number of indexes from our data.
	...
	\item We can return a label array with a label based on the cluster they ended in.
...
In contrast to the partitioning clustering like K-Means, \emph{hierarchical clustering} creates different divisions in each iteration
...
The bigger the mutual information, the more similar.
...
methods such as \emph{silhouette plots} and \emph{prediction strength} to analyze the quality of our clusters.
...
the mean distance from \(x_i\) to all other points in the cluster.
...
the minimal mean distance from \(x_i\) to all other points in the closest cluster.

Finally, with these two concepts we can define the \emph{silhouette-index} as
...
\[
	\text{average silhouette width}(C_j) = \overline{s_j} = \frac{1}{|C_j|} \sum_{x \in C_j} s(x),
\]

\[
	\text{silhouette score}= \overline{s} = \frac{1}{n} \sum_{i = 1}^{n} s(x_i).
\]
...
the belonging to a specific cluster can be well predicted.
...
This means that the entry is 1 if two data-points of our validation set are in the same cluster with respect to our training-centroids.
