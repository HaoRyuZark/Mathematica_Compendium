...
We can use the the term \(\inner{x}{u}\) as the measure of \textbf{information preserved between the two vectors \(x\) and \(x'\)}. This is
...
\[
	\max_{\metric{u} = 1} \sum_i \inner{x_i}{u}^2 = \max_{\metric{u} = 1} \sum_i (x_i^T u)^2
\]
...
\begin{align*}
	\max_{\metric{u} = 1} \sum_i (x_i^T u)^2 & = \max_{\metric{u} = 1} \sum_i (x_i^T u)(x_i^T u)             \\
	                        & = \max_{\metric{u} = 1} \sum_i u^T x_i x_i^T u                \\
	                        & = \max_{\metric{u} = 1} u^T \left( \sum_i x_i x_i^T \right) u
\end{align*}
...
For our next component we would want to be in the orthogonal complement of the first component, so that it is uncorrelated with the first component. 
This means that we want to choose the eigenvector corresponding to the second largest eigenvalue, and so on.
...
We use the variance formula because it measeures the spread of the data along the direction of the principal component. The larger the variance, the more information is preserved in that direction.
