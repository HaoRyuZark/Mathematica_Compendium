...
One more use case of cross validation is to estimate \emph{tunning parameter}, which are not directly optimized by  the model, but determined by the person 
performing the analysis.

\subsection{Leave One Out Cross Validation}

This is cross validation but with leaving always \textbf{one data point out}, instead of a fold.
...
maximal when they are orthogonal and maximal when they are parallel. Hence we want to maximize the sum of the inner products between the original data and the projected data.
...
Finally reconstruct \(X\) from using te eigendecomposition with \(V\) as the matrix of eigenvectors and \(\Delta\) the eigenvalue matrix:
