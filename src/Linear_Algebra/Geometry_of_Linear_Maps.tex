...
\[
    \langle f(x), f(y) \rangle = \langle x, y \rangle \quad \forall x,y \in \R^n
\]
...
two matrices \(Q \text{ and } R\) such that \(A = QR \text{ with } R\in \R^{n \times n} 
\text{ and } Q \in \R^{m \times n}\). \emph{R} is matrix with only an upper triangle and 
