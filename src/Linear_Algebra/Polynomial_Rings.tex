...
Given a field \(\field\) with \(\field^{\Z_{\ge 0}}\), we define the multiplication 
...
\[
    c_n := \sum_{i = 0}^{n} a_i b_{n - i} = a_0 b_{n} + a_1 b_{n - 1} + \cdots + a_n b_0
\]
...
This \((\field^{\Z_{\ge 0}}, +, \cdot)\) is also noted as \(\field[[\field]]\) and 
...
for \(X := (0, 1, 0, 0, \dots)\). We also call this ring the \emph{formal power series} over \(\field\).  
...
Also, note that we define \(\deg(0):= -\infty\).
...
\[
    X_a = (0, a_0, a_1, \dots) \quad \forall a \in \field [[X]].
\]
...
\section{Field Algebra}

Given a ring \(R\) with a one that is also a vector space over the field \(\field\), we call 
\(R\) an \emph{associative algebra} with the one if and only if 
...
\section{Division for Polynomials}
...
use the commutativity of our field \(\field\). 
...
For the case \(m \ge n\), we will show the existence of \(q,r\) by induction on \(m\). For 
\(m = 0\) and \(n = 0\), \(f = a_0\), \(g = b_0\). Set \(q = \frac{a_0}{q_0}\) and \(r = 0\), and we get 
\(f = qg + r\).

Let \(m > 0\), and assume that for each polynomial \(h\) with \(\deg(h) \le m - 1\) there 
exist polynomials \(\hat{q}, \hat{r}\) such that \(\deg(\hat{r}) < \deg(g)\) and 
\(h = \hat{q}g + \hat{r}\).
...
\section{Greatest Common Divisor for Polynomials}

For two polynomials \(f,g \in \field[X]\) with \((f,g) \ne (0,0)\), the polynomial \(t \in \field[X]\) with 
...
is considered the \emph{greatest common divisor} of \(f\) and \(g\). If \(t = 1\),  then \(f,g\) are \emph{relatively prime}.

\section{More Properties of Polynomials}
...
\begin{itemize}
    ...
    \item The gcd of two polynomials \(f, g \in \field[X]\) with \((f,g) \neq (0,0)\) is unique only up to factors in \(\field[X]^{\ast}\), 
    that is, up to factors in \(\field[X]\) of degree \(0\). Notation: \(t = \gcd(f,g)\) actually also denotes every multiple \(a t\) with 
    \(a \in \field \setminus \{0\}\) as a gcd of \(f\) and \(g\).

    \item In both rings there is a division with remainder, and therefore \(\field[X]\) also has a Euclidean algorithm for computing 
    the gcd and an extended Euclidean algorithm for determining Bézout coefficients. Here the degree of a polynomial plays the role of the 
    absolute value in \(\Z\). We give the first part of the algorithm again.

    \item Furthermore, one also has the analogue of prime numbers in \(\field[X]\), namely irreducible polynomials, and correspondingly a unique prime 
    factorization for polynomials.

\end{itemize}

\section{Euclidean Algorithm for Polynomials}
...
A polynomial \(p \in \field[X] \setminus \{0\}\) is called \emph{irreducible} if \(p\) is not a unit (that is, \(p \notin \field[X]^{\ast}\)) and if for 
\(f, g \in \field[X]\) the following holds: \(p = f \cdot g \Rightarrow f \in \field[X]^{\ast} \,\vee\, g \in \field[X]^{\ast}\). An element \(h \in \field[X] \setminus \{0\}\) for which 
\(h = f \cdot g\) holds with \(f, g \in \field[X] \setminus \field[X]^{\ast}\)  is called \emph{reducible}.

\section{Normalized}

A polynomial \(f \in \field[X] \setminus \{0\}\) with \(f = \sum_{i = 0}^{n} a_i X^i\) is considered \emph{normalized} if \(a_n = 1\).

\section{Factorization Property}

Each polynomial \(f \in \field[X] \setminus \{0\}\) has, up to the order of the factors, a unique factorization into a normalized irreducible form and 
a unit.