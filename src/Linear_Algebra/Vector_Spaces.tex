...
Given a field \(\field\), a \emph{vector space} is a set \((V, \field, \oplus , \odot)\) with two operations, vector addition and scalar multiplication, 
such that:

\begin{enumerate}[label=\Roman*.]

	\item \((a \oplus b)\vec{v} = a\vec{v} \oplus b\vec{v}, \forall a,b \in \field \land \vec{v} \in V\)
	
	\item \((\vec{v} \oplus \vec{u})a = a\vec{v} \oplus a\vec{u}, \forall a\in \field \land \vec{v}, \vec{u} \in V\)

	\item \((ab)\odot \vec{v} = a(b \odot\vec{v}), \forall a,b \in \field \land \vec{v} \in V\)

	\item \(1 \odot \vec{v} = \vec{v}, \forall \vec{v} \in V\)
...
\[
	\field \times  V \to V: (a,v) \mapsto a \odot v.
\]
...
\begin{itemize}
	...
	\item \(\vec{v} \odot 0 = \vec{0}\)

	\item \((-1) \odot \vec{v} = -1 \odot \vec{v}\)
...
\begin{enumerate}
	...
	\item The set of all \(n\)-tuples of real numbers \(\R^n\) is a vector space over the field 
	      of real numbers \(\R\).

	\item The set of all polynomials of degree less than or equal to \(n\) is a vector space over the 
	      field of real numbers \(\R\).

	\item The set of all continuous functions from \(\R\) to \(\R\) is a vector space 
...
A subset \(W\) of \(V\) is a subspace if and only if \(W \ne \emptyset \) and, for all 
\(v,w \in W\) and all \(a \in \field\), \(v + aw \in W\).

\textbf{Proof:}

\(\Rightarrow :\) \(\vec{0} \in W\) thus \(W \ne \emptyset\). Also, by the axioms 
\(v + aw \in W\).

\(\Leftarrow:\) \(W \ne \emptyset \implies \exists w \in W\) because \(\field\) is a field; then \(-1 \in \field\) and therefore,
\(w - w = 0 \in W\). For \(v + aw \in W\) we choose \(a = 1\) or \(v = 0\), and we see that the addition and multiplication are also defined
for our vector space.

\subsection{Properties of the subspaces}

\begin{itemize}

	\item The intersection of two or more subspaces \(\bigcap_{W \in V} W \) is a subspace.
...
\begin{itemize}
	...
	\item The \emph{direct sum} of two subspaces \(U\) and \(W\) is defined as:
	\[
...
\section{Vector Space Product}

The \emph{product} \(\langle M \rangle\) of \(M \subseteq V\) is the intersection of all subspaces of \(V\) which contain \(M\):

\[
	\langle M \rangle := \bigcap_{{W \subseteq V}_{M \subseteq W}} W.
\]
...
is called the \emph{span} of those vectors, denoted by \(\mathrm{span}(\vec{v}_1, \vec{v}_2, \ldots, \vec{v}_n)\). The span of a set of vectors is a 
subspace of the vector space \(V\).

\begin{itemize}

	\item The span of a set of vectors is the smallest subspace containing those vectors.

	\item The set of all linear combinations of a set of vectors \(B = \{\vec{v}_1, \vec{v}_2, \ldots, \vec{v}_n\}\) is given by \(\mathcal{L}(B)\). 
...
is \(a_1 = a_2 = \cdots = a_n = 0\). If there exists a non-trivial solution to this equation, then the set of vectors is said to be linearly dependent. 
A set of vectors is linearly independent if and only if the only linear combination of those vectors that equals the zero vector 
is the trivial combination where all coefficients are zero.

\subsection{Properties Of The Linear Independence}
...
A \emph{basis} of a vector space \(V\) is a set of vectors 	\(\{\vec{v}_1, \vec{v}_2, \ldots, \vec{v}_n\}\) that is linearly independent and 
spans the vector space \(V\).

\subsection{How to find the basis of a set of vectors}
...
\begin{enumerate}
	...
	\item The non-zero rows of the echelon form matrix correspond to the basis of the vector space 
...
\subsection{Basis Isomorphism}

Given \(B = (v_1, \dots, v_n)\). \(B\) is a basis of \(V\) if and only if 

\[
	\lambda = \lambda_B: \field^n \to V: (a_1, \dots, a_n) \mapsto \sum_{i=1}^n a_i v_i
\]
...
The \emph{dimension} of a vector space \(V\) is the number of vectors in a basis of \(V\).
...
\[
	\mathrm{span}(v_1, \ldots, v_r, w_1, \ldots, w_s) = V.
\]
...
If \(\mathrm{span}(v_1, \ldots, v_r) = V\), the statement is obvious. So assume

\[
	\mathrm{span}(v_1, \ldots, v_r) \neq V.
\]

Then there exists at least one \(w_i\) such that \(w_i \notin \mathrm{span}(v_1, \ldots, v_r)\); 
otherwise, if all \(w_i \in \mathrm{span}(v_1, \ldots, v_r)\), then

\[
	\mathrm{span}(v_1, \ldots, v_r, w_1, \ldots, w_s) = \mathrm{span}(v_1, \ldots, v_r) = V,
\]

which contradicts our assumption that \(\mathrm{span}(v_1, \ldots, v_r) \neq V\).
...
It follows that \(\lambda = 0\) (since \(w_i \notin \mathrm{span}(v_1, \ldots, v_r)\)), and then also 
...
\subsection{Epimorphism of Finite Dimensional Vector Spaces}

A finite dimensional vector space is an \emph{epimorphic} image of \(Field^n\) where \(n\) is the number of dimensions.
...
\[
	\lambda: K^n \to V: a \mapsto a_1 X_1 + \cdots + a_n X_n
\]

\subsection{Exchange Lemma}

Let \((v_1, \ldots, v_n)\) be a basis of \(V\) and \((w_1, \ldots, w_m)\) be a generating system of \(V\). Then, for every 
\(v_i\), there exists a \(w_j\) such that if we replace \(v_i\) by \(w_j\) in the tuple 
\((v_1, \ldots, v_n)\), it still forms a basis of \(V\). We can exchange \(m - n\) vectors in this way.
...
\[
	\mathrm{span}(v_1, \ldots, v_{i-1}, v_{i+1}, \ldots, v_n) \neq V,
\]

because if \(\mathrm{span}(v_1, \ldots, v_{i-1}, v_{i+1}, \ldots, v_n) = V\), then \(v_i\) would lie in the 
...
\[
	w_j \notin \mathrm{span}(v_1, \ldots, v_{i-1}, v_{i+1}, \ldots, v_n),
\]
...
For \(m = 1\) \(A \cup B = (v_1, \dots, v_n, w_1)\) is linearly dependent because \(A\) is a basis of \(V\). 
Thus, there exist \(\lambda_1, \dots, \lambda_n = w_1\). 
...
\[
	\tilde{\lambda_1} v_1 + \dots + \tilde{\lambda_{j-1}} v_{j-1} +\tilde{\lambda_j} w_1+ \tilde{\lambda_{j+1}} v_{j+1} + \dots + \tilde{\lambda_n} v_n = 0
\]

Assume \(\tilde{\lambda_j} \ne 0\) then we can express \(w_1\) as a linear combination of the other vectors. This is a contradiction because this would mean 
that we can write \(w_1\) as another linear combination of the vectors in \(A\) which is not possible because \(B\) is a basis of \(V\). 

If we repeat this process \(n - m\) times, we get the desired result. 

\QED

\section{Dimension As A Sum Of The Dimension Of Subspaces}
...
\begin{enumerate}
	...
	\item Therefore, we have \(a_0 = a_1 = \cdots = a_n = 0\), which proves that the set of polynomials \(\{1, x, x^2, \ldots, x^n\}\)
	is linearly independent.

\end{enumerate}

So you only have to prove that the set of coefficient vectors is linearly independent.

\section{Interpolation Polynomial}

Given the \(n+1\) points \((x_k, y_k)\), with \(0 \leq k \leq n\) and all \(x_k\) distinct, there exists exactly one polynomial \(p_n \in P_n\) such 
that \(y_k = p_n(x_k)\) for all \(0 \leq k \leq n\). This polynomial is called the interpolation polynomial.

\textbf{Proof:}

The uniqueness follows immediately. We prove the existence by induction on \(n\). For \(n = 0\), choose \(p_0(x) = y_0\). 

Now assume the statement is true for \(n-1\). Let the polynomial \(p_{n-1}\)interpolate the points \((x_0, y_0), \ldots, (x_{n-1}, y_{n-1})\). 
...
We have \(q \in P_n\), and it follows that \(p_n \in P_n\). Furthermore, \(q(x_k) = 0\) for \(k \leq n-1\) because a linear factor in 
the numerator always vanishes at \(x_k\). Therefore, \(p_n(x_k) = y_k\) for \(k \leq n-1\). Additionally, we have
...
Consider the three points \((-2, 1)\), \((-1, -1)\), and \((1, 1)\). These points uniquely define an interpolating parabola \(p_2\). This parabola can be 
determined using the definition of \(p_n\). For hand calculations and a few points to interpolate, the following approach is also useful. The general form of 
the polynomial is 
