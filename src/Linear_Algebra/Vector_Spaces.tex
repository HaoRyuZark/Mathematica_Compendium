...
\begin{enumerate}
	...
	\item The set of all continuous functions from \(\R\) to \(\R\) is a vector space 7
...
\(v,w \in W\) and all \(a \in \field\) then \(v + aw \in W\)
...
\(w - w = 0 \in W\). For \(v + aw \in W\) we choose \(a = 1\) or \(v = 0\), and we see that the addition and multiplication is also defined
...
\subsubsection{Vector Space Product}

The \emph{product} \(\langle M \rangle\) of \(M \subseteq V\) is the intersection of 
all subspaces of \(V\) which contain \(M\)
...
The set of all linear combinations of a set of vectors \(B = \{\vec{v}_1, \vec{v}_2, \ldots, \vec{v}_n\}\) is given by \(\mathcal{L}(B)\)
...
\begin{itemize}
	...
	\item A \emph{basis} of a vector space \(V\) is a set of vectors 
	      \(\{\vec{v}_1, \vec{v}_2, \ldots, \vec{v}_n\}\) that is linearly independent and 
		  spans the vector space \(V\).

	\item The number of vectors in a basis of a vector space is called the \emph{dimension} of the 
	      vector space.

	\item The dimension of a vector space \(V\) is denoted by \(\dim(V)\).

	\item If \(V\) has a finite basis, then it is said to be \emph{finite-dimensional}.

	\item If \(V\) does not have a finite basis, then it is said to be \emph{infinite-dimensional}.
...
\subsubsection{How to find the basis of a set vector}
...
Let \((v_1, \ldots, v_n)\) be a base of \(V\) and \((w_1, \ldots, w_m)\) be generating system of \(V\). Then, for every 
