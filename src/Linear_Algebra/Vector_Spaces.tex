...
The basis of a vector space is not unique. Different basiss can span the same vector space.
...
Let \((v_1, \ldots, v_n)\) and \((w_1, \ldots, w_m)\) be basiss of a vector space \(V\). Then, for every 
\(v_i\), there exists a \(w_j\) such that if we replace \(v_i\) by \(w_j\) in the tuple 
\((v_1, \ldots, v_n)\), it still forms a basis of \(V\).

\textbf{Proof:}

Let \((v_1, \ldots, v_n)\) and \((w_1, \ldots, w_m)\) be two basiss of \(V\). Suppose we remove \(v_i\) 
