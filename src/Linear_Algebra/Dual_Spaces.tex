...
Also a linear functional \(\hat{v} \in V^{**}\) is defined by \(\hat{v}(f) = f(v)\) for all \(f \in V^*\).
...
Let \(\phi: V \to V^{**}\). It takes an input \(v \in V\) and outputs a linear functional \(\hat{v}\) defined by 
\(\hat(v): V^* \to \Field\). 
