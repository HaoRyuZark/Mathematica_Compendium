...
Each of them also encodes the row operations from which they came.
...
\begin{align*}
     C_2:= \text{Swap}_m (i,j) :=&  \\ 
    & (e_1, \dots, e_{i - 1}, e_j, e_{j + 1}, \dots, e_{j - 1}, e_i, e_{j + 1}, \dots, e_m): m \times m \to \field : (s,t) \mapsto \\
    &\begin{cases}
        1, & s = t \notin \{i, j\} \\ 
        1, & (s, t) \in \{(i,j), (j,i)\}\\ 
        0, &\text{ else} 
...
Because every row operation is \emph{invertible}, we can now also claim that elementary matrices are invertible and that they also represent 
that inverted row operation.
