...
\begin{enumerate}
    ...
    \item Take current diagonal element and proceed to divide all entries in the same column underneath by it. Do not forget, that if we are in a sub-matrix.
    ...
    are going to use for our row at each subtraction is going to be first entry of the current column inside our sub-matrix.
    
    \item We reduce the scope of our matrix and repeat the steps above until we have a 1 by 1 matrix.
    
    \item After the previous process, the resulting matrix will be divided into two matrices. \(L\) consisting on 
...
modified to always take the row with biggest element in the current column, before perfoming the division step. In such a case we swap the entire row 
and not just the row inside our sub-matrix. 

We also need to keep track of the swaps which is accomplished by storing a vector \(x \in \R^n\)  where \(x_i\) is the position of \(i\)-row.
