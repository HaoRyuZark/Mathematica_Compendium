...
A \emph{linear system of equations} is a set of equations that can be expressed in the form:
...
More formally, given \(M, N\) with \(f: M \to N\), for each \(n \in N\) the corresponding 
\emph{linear system of equations} is given by 
...
There are the following types of solutions depending on the nature of \(f\):
...
where \(A\) is the coefficient matrix, \(x\) is the vector of variables, and \(b\) is the vector of constants. The coefficient matrix \(A\) is an 
\( m \times n \) matrix, where \( m \) is the number of equations and \(n\) is the number of variables. The vector \(x\) is an \( n \times 1 \) column
vector, and the vector \(b\) is an \( m \times 1 \) column vector.

\section{Solution Set Of A Linear System}

The solution set \( L(A, b) \) of the system of equations associated with \( (A, b) \) is defined as
...
For a matrix \(A \in \field^{m \times n}\) with rows \(Z_1, \dots, Z_n\), we define for \(i \in \underbar{m}\) the \(i\)-th \emph{echelon index} 
\(Ei_i(A)\) as 
...
\section{Equality of the Rank \texorpdfstring{\(\rank(A) = \rank_S(A)\)}{}}
...
\section{Solvability Of A Linear System \texorpdfstring{\(Ax = b\)}{}}
...
\section{Solution Set Of The Linear System \texorpdfstring{\(Ax = b\)}{}}
...
In a system of linear equations, a \emph{free variable} is a variable whose column in the coefficient matrix does not correspond to a pivot column 
after row reduction. Free variables are not directly solved by any equation in the system; instead, they can take any real value, and the values of 
the leading (or dependent) variables are expressed in terms of them.

Free variables arise naturally in systems with infinitely many solutions and play a key role in describing the solution set. Given a system with 
\(p\) equations and \(q\) unknowns, if \(q > p\) then there are \(q - p\) free variables.
...
Here, the pivot is in the first column corresponding to \( x_1 \), while \( x_2 \) and \( x_3 \) are free variables. Assigning parameters to \( x_2 \) 
and \( x_3 \), the general solution can be expressed in terms of these free variables.
...
These steps are defined as linear maps applied to a system like \(f(m) = n\) to \((g \circ f)(m) = g(n)\). We need to be able to find the respective inverses 
to get to the result \((g \circ f)^{-1}({g(n)}) = n\). This is accomplished via the \emph{elementary matrices}.

Once the matrix is in upper triangular form, back substitution is used to find the values of the variables. The last equation gives the value of the last 
variable, which can then be substituted into the previous equations to find the other variables.
...
where \(n\) is the index of the target row and \(i\) is the index of the column.
...
With \(\lambda\) and \(\gamma\) being complex sums of the results of the other manipulations.

\section{Invariability of the Solution Set}

Let \( f : M \to N \) and \( n \in N \). For every bijective map \( g : N \to N \), the following holds for the fiber of \( f \) over \( n \):
...
Let \( m \in f^{-1}(\{n\}) \), i.e.\ \( f(m) = n \). Then \( g(f(m)) = g(n) \), i.e.\ \( m \in (g \circ f)^{-1}(\{g(n)\}) \).
For the reverse implication one needs a left inverse of \( g \), which of course exists because \( g \) is bijective.

\QED 

\section{Gauss-Jordan Elimination}

\emph{Gauss-Jordan elimination} is an extension of Gaussian elimination that transforms the matrix into reduced row echelon form (RREF). In RREF, each 
leading entry in a row is 1, and all entries above and below the leading entry are zeros. The steps involved in Gauss-Jordan elimination are:
...
The Gauss-Jordan elimination method is particularly useful for finding the inverse of a matrix, as it can be applied to the augmented matrix \([A | I]\), 
where \(I\) is the identity matrix. If the left side of the augmented matrix becomes \(I\), then the right side will be the inverse of \(A\).
...
A \emph{particular solution} to a linear system of equations is a specific solution that satisfies the system. It can be found using various methods, 
including substitution, elimination, or matrix methods. A particular solution is not unique; there may be multiple particular solutions depending on the 
system. A particular solution can be found by substituting specific values for the variables and solving for the remaining variables. For example, in 
the system:
...
We want to prove that every linear system of equations \(Ax = b\) which has at least one solution fulfills 
...
For the first property, the solution of \(Ax = 0\) is \(\ker(A)\), more precisely \(\ker(\phi_A)\), and thus 
it is not empty because \(0 \in \ker(A)\).

The second proposition follows because the addition of the zero vector to a linear 
...
\section{Example: Gaussian Elimination with 3 Equations and 4 Unknowns}
...
\section{The Determinant, The Cross Product And The Solutions Of Linear Systems Of Equations}

A linear system of three equations has the following properties:
...
\section{Chart for the number of solutions}
...
\section{Proof of the Gaussian Elimination}
...
there would be some \(x \in L(CA,Cb)\) with \(CAx = Cb\). By multiplying both sides by \(C^{-1}\) we
...
If \(C\) is an elementary matrix and \(A \in \field^{m \times n}\) some matrix. The statement \(\rank(CA) = \rank(A)\)
...
and also the \(\rank_s (A)\). Concerning the scaling of a column by some \(\lambda \), \(L := L(a_1, \dots, \lambda a_k, \dots, a_n)\)
also preserves \(dim(L) = \rank_s(A)\). And finally, the addition \(L = L(a_1, \dots, a_i + \lambda a_k, \dots, a_n)\) also preserves
\(\rank_s (A) = \rank(A)\). Thus, \(\rank(CA) = \rank(A)\).
...
and then \(\rank(A^{(\ell)}) \le n - 1\) which is a contradiction to the assumption \(\rank(A^{(\ell)}) = n\).

\QED

\section{Alternative Method For Finding The Rank Of A Matrix}

For a matrix \(A \in \field^{(m \times n)}\). The biggest submatrix of size \(r \times r\) with a
non-zero determinant gives us the \emph{rank} of the matrix. \(\rank(A) = r\).

\section{Cramer's Rule}

For a square matrix \(A = (a_1, \dots, a_n)\) and \(x,b \in \field^{n}\) with \(Ax = b\) and \(\det A \ne 0\),
then
...
\(X_i = (e_1, \dots, e_{i - 1}, e_i, e_{i + 1}, \dots, e_n)\). Applying the Laplace formula \(n\) times along the first row shows that \(|X_i| = x_i\).
...
This implies \(|A_i| = |AX_i| = |A||X_i|\) therefore, \(x_i = \frac{|A_i|}{\det(A)}\).

\QED

Another way of thinking about this in a more geometric way is to first think of the determinant as the area/volume/etc. that is spanned by the basis 
vector and the stretching factor of the transformation \(\det(A)\). Now visualize your matrix \(A\) as a linear transformation with
a non-zero determinant and think about \(\det(A_i)\) as the area/volume/etc. which is spanned by the basis vector but with one of the vectors substituted 
by \(b\).

Now in two dimensions, think of the area spanned by the basis vectors \(A = xy\). The signed area of the original parallelogram gets
stretched by \(\det(A)\), which gives us \(Area = \det(A)y \implies y = \frac{Area}{\det(A)}\). This also works for \(x\).
...
of \(A\), where \(L\) is a lower triangular matrix with main diagonal consisting only of ones and 
\(U\) being an upper triangular matrix. \(a_{k,k}^{(k - 1)} \ne 0\) denotes the elements of the main diagonal for the 
...
Start with your \(n \times n\) matrix and perform the following steps across all of the 
entries in the main diagonal and its corresponding sub-square-matrix.

\begin{enumerate}

    \item Take the current diagonal element and proceed to divide all entries in the same column underneath by it. Do not forget that we may be working 
    within a sub-matrix.

    \item The current row of our matrix is going to be subtracted from all of the rows underneath inside our sub-matrix. The factor we 
    are going to use for our row at each subtraction is going to be the first entry of the current column inside our sub-matrix.
    
    \item We reduce the scope of our matrix and repeat the steps above until we have a 1 by 1 matrix.
    
    \item After the previous process, the resulting matrix will be divided into two matrices. \(L\) consisting of 
    the elements underneath the main diagonal with it being all ones and above all zeros; and \(U\) being the rest, the upper triangular matrix.
...
Now we subtract \(II - 2I\) and \(III - 1I\):
...
\subsection{Extended Algorithm for Any Regular Matrix}

We want to expand the algorithm to be compatible with the swap rows operations. Given a regular matrix 
\(A^{(k - 1)} = F_{k - 1} \cdot \dots \cdot F_1 A\), where after Gaussian operations we get \(a_{k,k}^{(k - 1)} = 0\), then 
there exists in the \(k\)-th row underneath at least one \(a_{i,k}^{(k - 1)} \ne 0, i > k\), so that we can swap the rows.

\textbf{Proof:}

Let us suppose that there does not exist such an element; then the \(k\)-th column must be a linear combination of the other 
...
with the following properties: \(P = P^T\) and \(PP = I\), which means \(P = P^{-1}\).
...
Now \(L\) is a lower triangular matrix, because of the following inductive proof: 

Let \(L_1 = F_{1}^{-1}\), \(L_k = P_k L_{k -1} P_k F_{k}^{-1}\) for \(k = 2, \dots, n - 1\). That is, \(L_{n  - 1} = L\).
...
In \(P_2 L_1\), the second and \(j\)-th rows are changed
...
Then in \(P_2 L_1 P_2\) the change gets undone; and finally, when multiplied by 
...
\subsection{Extended \(LU\)-Decomposition Algorithm}
...
\begin{enumerate}
    ...
    \item Solve \(Ly = Pb\). First permute \(b\) just like \(A\), and then solve \(Ly = Pv\).
...
We now use a vector \(p\) of size \(n \times 1\), where the \(j\)-th entry \(p_j\) is read as: switch row \(j\) with \(p_j\).
...
\subsection{Greatest Pivot Strategy}

As a side note, to mitigate the rounding errors in scientific computing, the swap row strategy is 
modified to always take the row with the biggest element in the current column, before performing the division step. In such a case we swap the entire row 
and not just the row inside our sub-matrix. 

We also need to keep track of the swaps, which is accomplished by storing a vector \(x \in \R^n\) where \(x_i\) is the position of the \(i\)-th row.

\section{Cholesky Decomposition}

This is a special case of the \(LU\)-Decomposition. Given \(A \in \R^{n \times n}\) a symmetric, positive definite matrix it can be decomposed as 

\[
    A = \tilde{L}D\tilde{L}^T,
\]

where \(\tilde{L}\) is a lower triangular matrix with 1's on the main diagonal and \(D\) a diagonal matrix with positive 
diagonal elements. (\emph{Rational Cholesky Decomposition}).

Or \(A = G G^T\), where \(G\) is a lower triangular matrix. (\emph{Cholesky Decomposition}).

\subsection{Algorithm for the Cholesky Decomposition}
...
\begin{enumerate}
    ...
    \item Determine \(V\) by taking \(L^T\), replacing its main diagonal with the diagonal of \(U\) and then dividing each row by the pivot 
    elements of \(D\). For symmetric matrices \(V = L^T\).

    \item Find the root of \(D\) by splitting it into two diagonal matrices which consist of the square roots of the elements in the main diagonal.
