...
In this chapter we are going to take a look at a special type of linear map that is called \emph{orthogonal} 
and at the term \emph{isometry}. It will be useful to remember some concepts about the dot product, such as 
\(\inner{x}{x} = x^T \cdot x = \metric{x}^2\).

\section{Isometry}

Given two vector spaces \((V, \metric{\cdot}_V)\) and \((W, \metric{\cdot}_W)\), a map 
\(f: V \to W\) is called \emph{isometric} if and only if \(\metric{x - y}_V = \metric{f(x) - f(y)}_W \).

It also has the following traits:

\begin{enumerate}

    \item \emph{Length-Preserving}: \(\metric{x}_V = \metric{f(x)}_W \quad \forall x \in V\).

    \item \emph{Angle-Preserving}: \(\angle (x,y) = \angle (f(x),f(y)) \quad \forall x,y \in V\).

    \item \emph{Linear}: \(f(x + \lambda y) = f(x) + \lambda f(y)\).

\end{enumerate}

\section{Unit-Sphere}

For \((V, \R)\) with metric \(\metric{\cdot}\) we denote 

\[
    S = \{x \mid x \in V : \metric{x} = 1\}
\]
...
we can rotate the vector \(\vec{v}\) by some \(\beta\) to transform it into \(\vec{g}\).
...
\subsection{Rotation Matrices for \texorpdfstring{\(\R^3\)}{}}

Note that a rotation is determined by two axes; hence, we get 3 different matrices depending on the two axes we need to rotate.
...
A matrix \(A \in \R^{n \times n}\) is \emph{orthogonal} if its column vectors form an orthonormal 
basis.
...
\begin{enumerate}
    ...
    \item \(\metric{Q}_2 = \sup_{x \ne 0} \frac{\metric{Qx}_2}{\metric{x}_2} = 1\).
...
\section{The Determinant of Orthogonal Matrices}

The \emph{determinant} of this kind of matrix can only be either positive 1 or negative 1.
...
Given \(f: \R^n \to \R^n\), these two statements are equivalent:
...
\(f\) preserves the angles and length. \(f\) is also called \emph{orthogonal}.
...
With \(\alpha = \angle(x, y) = \angle(f(x), f(y))\), it holds:
...
\subsection{Properties of Orthonormal Matrices}
...
We get a matrix with 1's at the entries \(QQ^t_{i,j}\) if \(i = j\) and 0 otherwise. In other words, the identity.

\QED

\textbf{Proof 2:}

For all \(x \in \R^n\) it holds that 

\[
    \|Qx\|_2^2 = (Qx)^T(Qx) = x^T Q^T Q x = x^T x = \|x\|_2^2.
\]

Hence, orthonormal matrices preserve the length. Therefore \(\|Q\|_2 = 1\), and 
analogously, the same holds for \(Q^{-1}\), hence 
...
\section{Theorem of the Matrix Representation}

The mapping \( f : \R^n \to \R^n \) is orthogonal if and only if  the associated matrix representation \( Q \) (with respect to the standard basis) 
is an orthogonal matrix, i.e., \( Q \in O(n) \).
...
\( Ax = b\) with a matrix \(A\in \R^{m \times n}\), 
...
\(m \ge n\) and \(\rank(A) = n\) we can find 
...
Because of the orthogonality of the columns \(\rank(Q) = \rank(A)\), therefore, it can be inverted. Hence,
we get:
...
\[
    R =
    \begin{bmatrix}
        \inner{q_1}{\vec{a}_1} & \inner{q_1}{\vec{a}_2} & \cdots &\inner{q_1}{\vec{a}_n} \\ 
        0 & \inner{q_2}{\vec{a}_2} & \cdots & \inner{q_2}{\vec{a}_n} \\ 
        \vdots & \vdots  & \ddots & \vdots \\
        0 & 0  & \cdots & \inner{q_n}{\vec{a}_n} \\ 
    \end{bmatrix}
\]
