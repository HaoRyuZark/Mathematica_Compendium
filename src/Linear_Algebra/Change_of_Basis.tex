...
Imagine you have a vector \( \vec{\lambda} \) in a vector space \( V \) with basis \( B_v \), and you also have another basis \( B_w \) 
for the same vector space.

The basis \( B_w \) is written in terms of the basis \( B_v \) as usual. The question is, given a vector written in the basis \( B_v \),
how is this vector expressed in the basis \( B_w \)? Note, that simply taking \(B_w \vec{v}\) will not give you the correct answer, as it 
will give another vector.

The key to getting the correct answer is to frame the question as: which vector \(\vec{\mu}\), when the transformation \(B_w\) is applied to it, 
will give us the vector \(\vec{v}\)? To answer this question, we need to take the inverse of \(B_w\) and apply it to \(\vec{\lambda}\). 
This will give us the vector \(\vec{\mu}\), which is the representation of \(\vec{v}\) in the basis \(B_w\), since
when we apply \(B_w\) to \(\vec{\mu}\), we will get back the vector \(\vec{\lambda}\).

Using the information above, we can write the following formula for translating between bases:
...
where \(\varphi_\beta\) and \(\varphi_\gamma\) are the matrices made of the basis vectors of the bases \(\beta\) and \(\gamma\) with respect
...
\section{Change of Basis Theorem}
...
\section{Change of Basis in Linear Maps}

Let \( V \) and \( W \) be finitely generated \( \field\)-vector spaces with bases \(\gamma\) and \(\beta\), respectively, and let \( f \in \mathrm{Hom}(V, W) \).
Then we have
...
Thus, \( M_\beta^\gamma(f) \) tells you to take a vector in the basis \(\gamma\), translate it to the canonical basis,
...
\section{Transformations Involving Multiple Bases}
...
We can visualize this as a commutative diagram where, when we take an inverse, we reverse the direction of the arrows:
