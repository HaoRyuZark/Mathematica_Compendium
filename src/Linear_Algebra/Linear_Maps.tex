...
A \emph{linear map} \( f: V \to W \) is a function that satisfies the following properties:

\begin{itemize}
    
    \item \( f(v_1 + v_2) = f(v_1) + f(v_2) \) for all \( v_1, v_2 \in V \).
    
    \item \( f(\lambda v) = \lambda f(v) \) for all \( v \in V \) and \( \lambda \in \field \).
    
    \item \( f(0) = 0 \).

\end{itemize} 

This kind of function is also called a \emph{homomorphism}. If \(V = W\), it is called an \emph{endomorphism}.
...
Two vector spaces are \emph{isomorphic} if there exists a bijective linear map between them. In this case, we can say 
...
\begin{itemize}
    ...
    \item Linear maps form a vector space 
...
\section{Composition as Matrix Multiplication}

Given \(A \in \field^{m \times n}, B \in \field^{n \times l}\) and  \(\phi_A: \field^{n \times 1} \to \field^{m \times 1}\) and 
\(\phi_B: \field^{l \times 1} \to \field^{n \times 1}\) be the induced linear maps, then 
...
\begin{itemize}
    ...
    \item \( \varphi_A \) is injective if and only if the fiber of \( \varphi_A \) over the zero column \( 0_m \in \field^{m \times 1} \)
    consists only of the zero column \( 0_n \in \field^{n \times 1} \).

    \item  \( \varphi_A \) is surjective if and only if every column in \( \field^{m \times 1} \) can be written as a linear combination of the columns 
    of \( A \).

\end{itemize}

\section{Linear Extension Theorem}

Let \(\beta = \{v_1, \dots, v_n\}\) be a basis of the finite dimensional vector space \(V\) and 
\(w_1, \dots, w_n\) be arbitrary vectors in \(W\). Then \(!\exists L\) with \(L : V \to W\) such 
that \(T(v_i) = w_i\).
...
\[
    U(x) = \lambda_1U(v_1) + \dots + \lambda_n U(v_n) =  \lambda_1T(v_1) + \dots + \lambda_n T(v_n) 
\]
...
\section{The Kernel of a Linear Map}

The \emph{kernel} of a linear map \( f: V \to W \) is the set of all vectors in \( V \) that are mapped to 
...
To compute the \emph{kernel} of the linear map, we just solve the homogeneous system of equations \(Ax = \vec{0}\) 
where \(A\) is the matrix representation of \(f\). The number of free variables is the \(\dim(\ker(f))\).

\textbf{Proof:}

\(\ker(f) \ne \emptyset \) because \(0 \in \ker(f)\) (at least zero should be in the kernel by the definition of the vector space and linear maps). 
Also, for \(v, w \in \ker(A)\) we know that \(f(av + bw) = af(v) + bf(w) = a0 + b0 = 0\). 

\QED

\section{The Nullity}

The \emph{nullity} of a linear map \( f: V \to W \) is the dimension of its kernel:

\[
    \mathrm{nullity}(f) = \dim(\ker(f))
\]

\section{The Image of a Linear Map}

The \emph{image} of a linear map \( f: V \to W \) is the set of all vectors in \( W \) that can be expressed 
...
\(f(0) \in \img(f) \implies \img(f) \ne \emptyset\). Therefore, for \(v_1,v_2 \in V, \exists w_1, w_2 \in \img(f)\) with 
\(w_1 = f(v_1)\) and \(w_2 = f(v_2)\) due to 

\[
    aw_1 + bw_2 = af(v_1) = bf(v_2) = f(av_1 + bv_2) \in \img(f).
\]

\QED 

\section{The Rank of a Linear Map}

The \emph{rank} of a linear map \( f: V \to W \) is the dimension of its image.

\[
    \rank(f) = \dim(\img(f))
\]

\section{The Nullity of a Linear Map}
...
\section{Proof of the Injectivity of the Kernel}
...
\section{Dimension Formula for Linear Mappings}
...
Remember that \(\ker(f)\) forms a subspace of \(V\) and therefore, \(\dim(\ker(f)) := r \leq n\). We extend an arbitrary basis \((v_1, \ldots, v_r)\) of 
\(\ker(f)\) to a basis \((v_1, \ldots, v_r, v_{r+1}, \ldots, v_n)\) of \(V\). Setting \(w_{r+i} = f(v_{r+i})\) for \(i = 1, \ldots, n - r\), we have 
\(\forall v \in V\):
...
We can state that matrices \emph{induce} linear maps, which we denote as \(\varphi_A\).

\section{How to Identify the Type of Linear Map}
...
\begin{itemize}
    ...
    That is, the image of \( f \) consists of the linearly independent vectors of \(f\).
...
Let \( f: \R^3 \to \R^2 \) be defined by the matrix
