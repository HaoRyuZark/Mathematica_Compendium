\chapter{Dual Spaces and Dual Vectors}

Given a \(\field\)-vector space \(V\), we define \(V^*\), the \emph{dual space} of \(V\), as the set
of linear transformations \(f: V \to K\). \(f\) is called the \emph{linear functional}.

Note, the vectors inside the space \(V^*\) are called \emph{dual vectors} or \emph{covariant vectors}, while vectors in the 
space \(V\) are called \emph{contravariant vectors}.

\textbf{Example:}

\(V = \R^3, \text{ with } f: V \to \R \quad \text{ and } \quad f(x,y,z) = x + y + z\)

\section{The Dual Basis}

Given a finite dimensional \(\field\)-vector space \(V\), let \(\beta = \{v_1, \dots, v_n\}\) be a
basis of \(V\), and let \(f \in V^*\). The set of functionals \(\beta^* = \{f_1, \dots, f_n\}\) of
\(V^*\), defined by:
...
is called a \emph{dual basis}. The definition above is saying that only one of the basis vectors
of \(V\) is going to be associated with a function of the basis \(\beta\).

\textbf{Proof:}

To prove a set of vectors is a basis, we have to prove that they are linearly independent and that
...
to be zero. Thus, the linear independence is shown.
...
The process for solving this kind of problem can be summarized in the following steps using
...
\begin{enumerate}
    ...
    \item Use the linearity property of vectors to write non-canonical basis vectors as a linear
    ...
    \item Use the definition of the dual basis for setting up the right-hand side of the linear system

    \item Solve the system of equations

    \item If an \(f\) was given, then use the found basis to write \(f = \sum_{i = 1}^{n} f(v_i)f_i\)

\end{enumerate}

\textbf{Example:}

Given the space \(V = \R^n\) with the basis \(\left\{\begin{bmatrix}
...
\section{Change of Basis in the Dual Space}

Given some finite \(\field\)-vector space \(V\) with basis \(\beta\) and \(\beta'\). Then the matrix for
the \emph{change of basis} \(\beta^* \to \beta'^*\) is given by
...
\section{Connection Between The Dual And The Transpose Matrix}
...
change of the transformation is given by \(A = [T]_{\beta}^{\gamma}\), which takes vectors from \(\beta\)
to \(\gamma\).
...
This sum is equal to \(A_{ji}\) when \(g_j(w_j)\). Therefore, we have flipped the columns
...
Because we have shown it for the basis vectors, it gets extended to all of its linear combinations.

\QED

\textbf{Example:}

Let \(P_1 \R\) be the space of polynomials of degree at most 1 and \(W = \R^2\). Also,
...
To find the matrix of \(T^T\), which is a \(2 \times 2\) matrix
...
\section{The Dual of the Dual Space}
...
\subsection{Isomorphism of the Dual Space}

If \(V\) is finite dimensional, then \(V^{**}\) and \(V\) are isomorphic. 

\subsection{Isomorphism from \(V\) to \(V^{**}\)}

If \(V\) is finite dimensional, then the map \(\phi: V \to V^{**}\) is an isomorphism. 

\section{Basis Theorem for the Dual Space}

Every basis \(\beta^*\) of \(V^*\) is the dual basis of some basis \(\beta\) of \(V\).

\textbf{Proof:}

Let \(\beta^* = \{f_1, \dots, f_n\}\) be a basis of \(V^*\). We can define a basis \(\beta\) of \(V\) with the help of the following:
...
\(\beta^* = \{f_1, \dots, f_n\}\) for \(V^*\), \(\gamma^* = \{g_1, \dots, g_n\}\) for \(W^*\), \(\beta^{**} =\{\mathcal{F}_1, \dots, \mathcal{F}_n\}\) 
for \(V^{**}\) and finally \(\gamma^{**} = 
...
We define the \emph{Annihilator} of a subspace of \(V\) as 
...
Let \( \{w_1, \dots, w_{k - 1}, w_{k}, w_{k + 1}, \dots, w_n\}\) be a basis of \(W\) with dimension \(k\). 
Let us extend the basis to a basis of \(V\) by \(\beta = \{w_1, \dots, w_{k - 1}, w_{k}, w_{k + 1}, \dots, w_n\}\) with dimension \(n\).

Now let us claim that the vectors \(f_{k + 1}, \dots, f_{n}\) are a basis of \(W^{\circ}\). This implies that for every linear combination 
of vectors from \(v_1\) to \(v_k\) which equals \(w\). 

\[
    f_{k + 1}(w) = \sum_{i = 1}^{k} f_{k + 1}(\lambda_i v_i) = 0
\]

Therefore, \(f_{k + 1}, \dots, f_{n}\) are in \(W^{\circ}\). Now we have to show that they are linearly independent.
...
Therefore, we get that for some \(f\) in the annihilator we get a linear combination of our chosen basis. 

\QED

\section{The Duality Argument}
...
If \(T\) is not surjective we can assume that the image of it is a strict subset of \(W\) which we will call \(W'\). We can also define a function 
\(g \in W^*\) which is not 0 everywhere except for the vectors in \(W'\). 
...
This concludes the proof in one way because we have found some linear combination for the zero vector different from the trivial solution.

For the other way of the if and only if, we know that \(T^T\) is not injective. This means that multiple vectors get transformed into the 
zero vector under \(T^T\). Therefore, if suppose \(T\) is surjective and 

\begin{align*}
    T^T(g)(x) &= 0 \text{ for some } g \ne 0\\ 
        g(T(x)) &= 0, \quad \forall x \in V 
\end{align*}

Then for \(y = T(x)\) it is always 0, but because we have assumed that \(g \ne 0\), we have a contradiction. \(T\) must not be surjective, 
and by getting rid of the negation on both sides we have proven the statement.

\QED

\section{The Rank of A Transpose is Equal to the Rank of \texorpdfstring{\(A\)}{}}
...
The right part tells us that for some \(g \in \null(T^T)\), \(T^T(g) = f_0\). But because we can write 

\[
    T^T(g)(x) = g(T(x)) = 0 \text{ where } T(x) \in \img(T).
\]

We know that \(g\) is in the annihilator of the image of \(T\). 

With this knowledge we can now prove \(\rank(A) = \rank(A^T)\) using the rank nullity theorem 

\begin{align*}
    \dim(\null(T^T)) + \dim(\img(T^T)) &= \dim(W^*) = \dim(W) \\ 
    \dim((\img(T^{\circ})) + \dim(\img(T^T)) &= \dim(W) \\ 
\end{align*}

For \(\dim(W)\) we get 

\begin{align*}
    \dim((\img(T)^{\circ})) + \dim(\img(T)) &=  \dim(W) \\  
    \dim((\img(T)^{\circ})) &=  \dim(W) - \dim(\img(T)) 
\end{align*}
...
\begin{align*} 
    \dim((\img(T)^{\circ})) + \dim(\img(T^T)) &= \dim(W) \\ 
    \dim(W) - \dim(\img(T))  + \dim(\img(T^T)) &= \dim(W)\\ 
    \dim(\img(T^T))  &=  \dim(\img(T))  
\end{align*}
