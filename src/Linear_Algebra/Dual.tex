...
The vector inside the space \(V^*\) are called \emph{dual vectors} or \emph{covariant vectors} while vectors in the 
...
Now let us claim that the vector \(f_{k + 1}, \dots, f_{n}\) is a basis of \(W^{\circ}\). This implies that for every linear combination of vector from \(v_1\) to 
\(v_k\) which equals \(w\). 
...
\subsection{The Duality Argument}
...
For the other way of the if and only if, we know that \(T^T\) is not injective. This means that multiple vector get transformed into the zero vector under \(T^T\).  
Therefore, if suppose \(T\) is surjective and 
...
Then for \(y = T(x)\) is always 0 but because we have assume that \(g \ne 0\) we have a contradiction. \(T\) must not surjective and by getting rid of 
the negation on both sides we have proven the statement.

\QED

\subsection{The Rank A transpose is equal to the rank of A}
...
With this knowledge we can know proof \(\rank(A) = \rank(A^T)\) using the rank nullity theorem 
