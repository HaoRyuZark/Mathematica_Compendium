...
In this section, we will cover the topics for the geometry of \(\R^2\) and \(\R^3\).
...
A point in \(\R^3\) is represented as \(\vec{P} = \begin{bmatrix} x \\ y \\ z \end{bmatrix}\).
...
The inner product of vectors \(\vec{a}\) and \(\vec{b}\) is a function \(\inner{x}{y} :=V \times V \rightarrow \mathbb{K}\)
...
\[
	\inner{v}{u} = v_1u_1 + v_2u_2 = \|v\|\|u\|\cos \theta \\ 
\]

\QED

And if we manipulate this expression we can also prove the angle formulas.
And with that information we can prove that

\[
	\cos \frac{\pi}{2}\ = \frac{\inner{v}{u}}{\|v\|\|u\|} = 0
\]
...
The \emph{orthogonal projection of vector} \(\vec{a}\) onto vector \(\vec{b}\) is given by:

\[
	\mathrm{Proj}_{\vec{b}}(\vec{a}) = \frac{\langle\vec{a}, \vec{b}\rangle}{\metric{\vec{b}}^2} \vec{b}
\]
...
Then let us define the orthogonal projection of vector \(\vec{a}\) on vector \(\vec{b}\) as \(p\). 
We want to find \(\alpha\) such that \(p = \alpha q\) where \(q = \vec{b}\).
We also know that the vector \(a - p\) is orthogonal to \(q\):
...
\begin{figure}[H]
	...
	\begin{tikzpicture}[scale=2, >=Stealth]
		...
		\draw[->, thick, green!60!black, dashed] (O) -- (P) node[above left] {\(\text{Proj}_{\vec{b}}(\vec{a})\)};

		% Orthogonal component
		\draw[->, thick, gray, dashed] (P) -- (A) node[midway, right] {\(\vec{a} - \text{Proj}_{\vec{b}}(\vec{a})\)};
...
visualization. For example, in Principal Component Analysis (PCA), data points are projected onto the directions of maximum variance to reduce the dimensionality 
of the dataset while preserving as much information as possible.

We find a unit vector in the direction of interest (e.g., a principal component) \(\vec{u_i}\) onto which we want to project our data points \(\vec{x_j}\). Note 
we can find multiple such vectors \(\vec{u_1}, \vec{u_2}, \dots , \vec{u_k}\) to project onto a lower-dimensional subspace.
...
\[
	p = k \vec{u} \quad \text{ and } d = \vec{v} - p
\]
...
The \emph{dot product} of two vectors can be interpreted as projecting one vector onto the other and then multiplying the length of the projecting vector to the magnitude of the other. For real vectors.
...
\subsection{Metrics for Vectors}
...
The cross product of two vectors \(\vec{a}\) and \(\vec{b}\) in \(\R^3\) is defined as:
...
\begin{itemize}
	...
	\item \(\vec{a} \times \vec{b} = \vec{0} \Leftrightarrow \vec{a} = \lambda\vec{b}\) for some \(\lambda \in \R\) (parallel vectors)

	\item \(\vec{a} \times \vec{b} = \vec{0} \Leftrightarrow \vec{a} = \vec{0} \vee \vec{b} = \vec{0}\) (zero vector)

\end{itemize}

The cross product is not defined in \(\R^2\). The cross product is not commutative, but it is associative:
...
For two vectors \(\vec{a}\) \(\vec{b}\) in \(\C^3\) we can compute the cross product by: 
...
\subsection{Orthogonal vectors in \texorpdfstring{\(\R^2\ \R^3\)}{}}

\subsubsection{Orthogonal vectors in \texorpdfstring{\(\R^2\)}{}}
...
\subsubsection{Orthogonal vectors in \texorpdfstring{\(\R^3\)}{}}
\begin{enumerate}
