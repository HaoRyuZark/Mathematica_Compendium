...
A natural number \(a\) has a \emph{perfect square} if \(\sqrt{a} \in \N\).
...
A number \(a \in \Z\) is divisible a number \(b \in \Z\) if 
...
Given \(a,b \in \Z\). There exists unique \(q,r \in \Z\) such that 
...
``\(\Rightarrow\)'' Let \(p\) be a prime and \(a,b \in \Z\)
...
``\(\Leftarrow\)'' Let \(c \in \N\) be a divisor of \(p\). Then there exists
\(d \in \N\) with \(p = cd\). In particular \(1 \le c \le p\) and
...
Write \(1 = \gcd(m, p) = am + bp\) for some \(a, b \in \Z\). Then
...
Given \(n \in \N\) has \(1\) as a prime factor. Suppose that every natural number 
smaller than \(n\) can be factorized into primes. 

If \(n\) is a prime number then it is itself its prime factorization. Else. there 
are \(b, c \in \Z\) with \(n = bc\) \(1 < b, c<n\). By our induction's supposition, 
