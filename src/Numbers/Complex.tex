...
\section{Operations in Cartesian Coordinates}
...
\section{Exponentiation and Roots (De Moivre’s Theorem)}
...
\section{Solving Equations with Complex Numbers}
...
\subsection{Equations Involving the Conjugate}
...
\subsection{Equations Involving \texorpdfstring{\( z \cdot \overline{z} \)}{}}
...
\subsection{General Polynomial Equations}
 ...
 Use De Moivre’s Theorem or polar form. 
...
The \emph{logarithm} of a complex number is multivalued due to the periodic nature of the complex exponential.
...
The \emph{principal value} of the complex logarithm is often written:
...
The process above is called \emph{linearization of the exponential function} by zooming \(\alpha \approx \frac{dy}{d}\).
...
Now imagine that in a sector of a circumference you put triangles one above the other with a base of 
length one, a height of \(\frac{C}{N}\), and an angle of \(\delta\).
...
\section{Complex Numbers as Matrices}

Given a complex number \( z = a + bi \), we can represent it as a real 2x2 matrix:

\[
	z \leftrightarrow \begin{bmatrix}
		a & -b \\
		b & a
	\end{bmatrix},
\]

with the linear function \( f: M_2(\R) \to \C \) defined by
...
Taking the determinant of this matrix gives us the modulus squared of the complex number:

\[
	\det\begin{bmatrix}
		a & -b \\
		b & a
	\end{bmatrix} = a^2 + b^2 = |z|^2
\]
