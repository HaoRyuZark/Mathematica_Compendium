...
We will now define the set of natural numbers, \( \N \), via the following 9 axioms. 
These axioms are known as the \emph{Peano Axioms}. The first 4 axioms define equality on 
the set \( \N \).

\begin{enumerate}[label=\Roman*.]

	\item For every \( x \in \N \), we have \( x = x \).\hfill (Reflexivity)
	
	\item For every \( x, y \in \N \), if \( x = y \) then \( y = x \).\hfill (Symmetry)
	
	\item For every \( x, y, z \in \N \), if \( x = y \) and \( y = z \) then \( x = z \).\hfill 
	      (Transitivity)
	
	\item For all \( x, y \), if \( x \in \N \) and \( x = y \), then \( y \in \N \).\hfill 
	      (Closure of Equality)
